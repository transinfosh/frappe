...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
    ...
    modules = frappe.get_all("Module Def", filters={"app_name": app_name}, pluck="id")
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
    ...
    for module_id in modules:
        ...
        for doctype in frappe.get_all("DocType", filters={"module": module_id}, fields=["id", "issingle"]):
            ...
            if not dry_run:
                if doctype.issingle:
                    frappe.delete_doc("DocType", doctype.id, ignore_on_trash=True, force=True)
                else:
...
def init_singles():
    singles = frappe.get_all("DocType", filters={"issingle": True}, pluck="id")
