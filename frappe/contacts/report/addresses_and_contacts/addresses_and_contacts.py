...
def get_data(filters):
    ...
    return get_reference_addresses_and_contact(reference_doctype, reference_id)


def get_reference_addresses_and_contact(reference_doctype, reference_id):
    data = []
    filters = None
    ...
    if reference_id:
        filters = {"id": reference_id}

    reference_list = [
        d[0] for d in frappe.get_list(reference_doctype, filters=filters, fields=["id"], as_list=True)
    ]
    ...
    for reference_id, details in reference_details.items():
...
def get_reference_details(reference_doctype, doctype, reference_list, reference_details):
    filters = [
        ...
        ["Dynamic Link", "link_id", "in", reference_list],
    ]
    fields = ["`tabDynamic Link`.link_id", *field_map.get(doctype, [])]
