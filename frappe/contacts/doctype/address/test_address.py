...
class TestAddress(IntegrationTestCase):
    def test_template_works(self):
        ...
        address = frappe.get_list("Address")[0].id
        display = get_address_display(frappe.get_doc("Address", address).as_dict())
        self.assertTrue(display)

    def test_address_query(self):
        def query(doctype="Address", txt="", searchfield="id", start=0, page_len=20, filters=None):
            if filters is None:
                filters = {"link_doctype": "User", "link_id": "Administrator"}
