...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
    ...
    if not frappe.flags.in_import and is_standard and frappe.conf.developer_mode:
        ...
        # json
        export_to_files(record_list=[[doc.doctype, doc.id]], record_module=module, create_init=is_standard)

        return os.path.join(frappe.get_module_path(module), scrub(doc.doctype), scrub(doc.id), scrub(doc.id))
...
def export_customizations(
    ...
    custom = {
        "custom_fields": frappe.get_all("Custom Field", fields="*", filters={"dt": doctype}, order_by="id"),
        "property_setters": frappe.get_all(
            "Property Setter", fields="*", filters={"doc_type": doctype}, order_by="id"
        ),
        "custom_perms": [],
        "links": frappe.get_all("DocType Link", fields="*", filters={"parent": doctype}, order_by="id"),
        "doctype": doctype,
    ...
    if with_permissions:
        custom["custom_perms"] = frappe.get_all(
            "Custom DocPerm", fields="*", filters={"parent": doctype}, order_by="id"
        )
...
def scrub_dt_dn(dt: str, dn: str) -> tuple[str, str]:
    """Return in lowercase and code friendly ids of doctype and id for certain types."""
    return scrub(dt), scrub(dn)


def get_doc_path(module: str, doctype: str, id: str) -> str:
    """Return path of a doc in a module."""
    return os.path.join(get_module_path(module), *scrub_dt_dn(doctype, id))
...
def export_doc(doctype, id, module=None):
    """Write a doc to standard path."""
    from frappe.modules.export_file import write_document_file

    print(f"Exporting Document {doctype} {id}")
    module = module or frappe.db.get_value("DocType", id, "module")
    write_document_file(frappe.get_doc(doctype, id), module)


def get_doctype_module(doctype: str) -> str:
    """Return **Module Def** id of given doctype."""
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
    ...
    module_id = get_module_id(doctype, module, prefix, suffix, app)
...
def get_module_id(doctype: str, module: str, prefix: str = "", suffix: str = "", app: str | None = None):
...
def make_boilerplate(
    ...
    target_path = get_doc_path(doc.module, doc.doctype, doc.id)
    template_name = template.replace("controller", scrub(doc.id))
    ...
    with open(target_file_path, "w") as target, open(template_file_path) as source:
        ...
        controller_file_content = cstr(template).format(
            ...
            classname=doc.id.replace(" ", "").replace("-", ""),
