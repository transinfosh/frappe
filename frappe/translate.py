...
def get_lang_code(lang: str) -> str | None:
    return frappe.db.get_value("Language", {"id": lang}) or frappe.db.get_value(
...
def get_lang_dict():
    ...
    return dict(frappe.get_all("Language", fields=["language_name", "id"], order_by="creation", as_list=True))
...
def get_messages_for_app(app, deduplicate=True):
    ...
    if modules:
        if isinstance(modules, str):
            modules = [modules]
        filtered_doctypes = (
            frappe.qb.from_("DocType").where(Field("module").isin(modules)).select("id").run(pluck=True)
        )
        for id in filtered_doctypes:
            messages.extend(get_messages_from_doctype(id))

        # pages
        filtered_pages = (
            frappe.qb.from_("Page").where(Field("module").isin(modules)).select("id", "title").run()
        )
        for id, title in filtered_pages:
            messages.append((None, title or id))
            messages.extend(get_messages_from_page(id))
        ...
        ids = (
            ...
            .from_(report)
            .where((report.ref_doctype == doctype.id) & doctype.module.isin(modules))
            .select(report.id)
            .run(pluck=True)
        )
        for id in ids:
            messages.append((None, id))
            messages.extend(get_messages_from_report(id))
            for i in messages:
...
def get_messages_from_doctype(id):
    ...
    messages = [("DocType: " + id, message) for message in messages if is_translatable(message)]

    # extract from js, py files
    if not meta.custom:
        doctype_file_path = frappe.get_module_path(meta.module, "doctype", meta.id, meta.id)
    ...
    messages.extend(get_messages_from_workflow(doctype=id))
    return messages
...
def get_messages_from_workflow(doctype=None, app_name=None):
    ...
    for w in workflows:
        states = frappe.db.get_values(
            document_state,
            filters=document_state.parent == w["id"],
            fieldname="state",
            distinct=True,
            as_dict=True,
            order_by=None,
        ...
        messages.extend(
            [("Workflow: " + w["id"], state["state"]) for state in states if is_translatable(state["state"])]
        )
        states = frappe.db.get_values(
            document_state,
            filters=(document_state.parent == w["id"]) & (document_state.message.isnotnull()),
            fieldname="message",
            distinct=True,
            order_by=None,
            as_dict=True,
    ...
    for fixture in fixtures:
        if isinstance(fixture, str) and fixture == "Custom Field":
            custom_fields = frappe.get_all(
                "Custom Field", fields=["id", "label", "description", "fieldtype", "options"]
            )
            break
            ...
            custom_fields.extend(
                frappe.get_all(
                    ...
                    fields=["id", "label", "description", "fieldtype", "options"],
                )
            )
    ...
    for cf in custom_fields:
        for prop in ("label", "description"):
            if not cf.get(prop) or not is_translatable(cf[prop]):
                continue
            messages.append(("Custom Field - {}: {}".format(prop, cf["id"]), cf[prop]))
        if cf["fieldtype"] == "Selection" and cf.get("options"):
            messages.extend(
                ("Custom Field - Description: " + cf["id"], option)
...
def get_messages_from_page(id):
    """Return all translatable strings from a :class:`frappe.core.doctype.Page`."""
    return _get_messages_from_page_or_report("Page", id)


def get_messages_from_report(id):
    """Return all translatable strings from a :class:`frappe.core.doctype.Report`."""
    from frappe.gettext.extractors.utils import is_translatable

    report = frappe.get_doc("Report", id)
    messages = _get_messages_from_page_or_report(
        "Report", id, frappe.db.get_value("DocType", report.ref_doctype, "module")
    )
...
def _get_messages_from_page_or_report(doctype, id, module=None):
    if not module:
        module = frappe.db.get_value(doctype, id, "module")

    doc_path = frappe.get_module_path(module, doctype, id)

    messages = get_messages_from_file(os.path.join(doc_path, frappe.scrub(id) + ".py"))
...
def get_all_languages(with_language_name: bool = False) -> list:
    ...
    if with_language_name:
        ...
        if not languages:
            languages = frappe.get_all("Language", filters={"enabled": 1}, pluck="id")
...
def get_translated_doctypes():
    dts = frappe.get_all("DocType", {"translated_doctype": 1}, pluck="id")
