...
@frappe.whitelist()
def bulk_workflow_approval(docids, doctype, action):
    docids = json.loads(docids)
    if len(docids) < 20:
        _bulk_workflow_action(docids, doctype, action)
    elif len(docids) <= 500:
...
def _bulk_workflow_action(docids, doctype, action):
    ...
    for idx, docid in enumerate(docids, 1):
        message_dict = {}
        try:
            show_progress(docids, _("Applying: {0}").format(action), idx, docid)
            apply_workflow(frappe.get_doc(doctype, docid), action)
            frappe.db.commit()
            ...
            if not frappe.message_log:
                ...
                failed_transactions[docid].append(message_dict)
            ...
            if not message_dict:
                if frappe.message_log:
                    ...
                    for message in messages:
                        ...
                        message_dict = {"docid": docid, "message": message.get("message")}

                        if message.get("raise_exception", False):
                            failed_transactions[docid].append(message_dict)
                        else:
                            successful_transactions[docid].append(message_dict)
                else:
                    successful_transactions[docid].append({"docid": docid, "message": None})
