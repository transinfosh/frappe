...
class BaseDocument:
    ...
    def _init_child(self, value, key):
        ...
        if not __dict.get("id"):
            __dict["__islocal"] = 1
            __dict["__temporary_id"] = frappe.generate_hash(length=10)
    ...
    def get_invalid_links(self, is_submittable=False):
        ...
        def get_msg(df, docid):
            # check if parentfield exists (only applicable for child table doctype)
            if self.get("parentfield"):
                return "{} #{}: {}: {}".format(_("Row"), self.idx, _(df.label, context=df.parent), docid)

            return f"{_(df.label, context=df.parent)}: {docid}"
        ...
        for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
            ...
            assert isinstance(docid, str | int) or (
                isinstance(docid, list | tuple | set) and len(docid) == 1
            ), f"Unexpected value for field {df.fieldname}: {docid}"
            ...
            if not meta.get("is_virtual"):
                values = frappe.db.get_value(
                    doctype, docid, values_to_fetch, as_dict=True, cache=True, order_by=None
                )
                if not values:  # NOTE: DB Value cache does negative caching, which is hard to remove now.
                    values = frappe.db.get_value(doctype, docid, values_to_fetch, as_dict=True, order_by=None)
            else:
            ...
            if not values.id:
                ...
                cancelled_links.append((df.fieldname, docid, get_msg(df, docid)))
