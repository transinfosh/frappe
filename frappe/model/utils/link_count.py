...
def notify_link_count(doctype, id):
    ...
    frappe.local._link_count[(doctype, id)] += 1
...
def update_link_count():
    ...
    if link_count:
        for (doctype, id), count in link_count.items():
            try:
                table = frappe.qb.DocType(doctype)
                frappe.qb.update(table).set(table.idx, table.idx + count).where(table.id == id).run()
                frappe.db.commit()
