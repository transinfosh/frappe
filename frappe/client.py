...
def bulk_update(docs):
    ...
    :param docs: JSON list of documents to be updated remotely. Each document must have `docid` property"""
    docs = json.loads(docs)
    failed_docs = []
    for doc in docs:
        doc.pop("flags", None)
        try:
            existing_doc = frappe.get_doc(doc["doctype"], doc["docid"])
...
@frappe.whitelist()
def has_permission(doctype: str, docid: str, perm_type: str = "read"):
...
@frappe.whitelist()
def get_doc_permissions(doctype: str, docid: str):
    ...
    """
    doc = frappe.get_lazy_doc(doctype, docid)
    return {"permissions": frappe.permissions.get_doc_permissions(doc)}


@frappe.whitelist()
def get_password(doctype: str, id: str, fieldname: str):
    ...
    return frappe.get_lazy_doc(doctype, id).get_password(fieldname)
...
def is_document_amended(doctype: str, docid: str):
...
@frappe.whitelist()
def validate_link(doctype: str, docid: str, fields=None):
    ...
    if is_virtual_doctype(doctype):
        try:
            ...
            frappe.msgprint(
                _("Document {0} {1} does not exist").format(frappe.bold(doctype), frappe.bold(docid)),
            )
        return values
    ...
    try:
        ...
        frappe.msgprint(
            _("You need {0} permission to fetch values from {1} {2}").format(
                frappe.bold(_("Read")), frappe.bold(doctype), frappe.bold(docid)
            ),
