...
@frappe.whitelist()
def optimize_doctype(doctype_id: str):
    frappe.only_for("System Manager")
    frappe.enqueue(
        optimize_doctype_job,
        queue="long",
        job_id=f"optimize-{doctype_id}",
        doctype_id=doctype_id,
        deduplicate=True,
    )


def optimize_doctype_job(doctype_id: str):
    from frappe.utils import get_table_name

    doctype_table = get_table_name(doctype_id, wrap_in_backticks=True)
