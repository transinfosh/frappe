...
def get_roles_and_doctypes():
    ...
    doctypes = frappe.get_all(
        ...
        filters={
            "istable": 0,
            "id": ("not in", ",".join(not_allowed_in_permission_manager)),
        },
    ...
    doctypes_list = [{"label": _(d.get("id")), "value": d.get("id")} for d in doctypes]
    roles_list = [{"label": _(d.get("id")), "value": d.get("id")} for d in roles]
...
def get_permissions(doctype: str | None = None, role: str | None = None):
    ...
    if role:
        ...
        if frappe.session.user != "Administrator":
            custom_roles = frappe.get_all("Role", filters={"is_custom": 1}, pluck="id")
