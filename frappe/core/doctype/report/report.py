...
class Report(Document):
    ...
    def create_report_py(self):
        if self.report_type == "Script Report":
            make_boilerplate("controller.py", self, {"id": self.id})
            make_boilerplate("controller.js", self, {"id": self.id})
    ...
    def execute_script(self, filters):
        ...
        safe_exec(self.report_script, None, loc, script_filename=f"Report {self.id}")
        if loc["data"]:
            return loc["data"]
        else:
    ...
    def build_standard_report_columns(self, columns, group_by_args):
        ...
        for fieldname, doctype in columns:
            ...
            if meta.get_field(fieldname):
                ...
                # since id is the primary key for a document, it will always be a Link datatype
                if fieldname == "id":
