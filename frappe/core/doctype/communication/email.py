...
def make(
    ...
    **kwargs,
    ...
    :param id: Reference Document id.
    ...
    :param print_format: Print Format id of parent document to be sent as attachment.
...
def _make(
    ...
    if attachments:
        ...
        add_attachments(comm.id, attachments)
    ...
    return {"id": comm.id, "emails_not_sent_to": ", ".join(emails_not_sent_to)}
...
def set_incoming_outgoing_accounts(doc):
    ...
    if doc.sent_or_received == "Sent":
        doc.db_set("email_account", doc.outgoing_email_account.id)


def add_attachments(id: str, attachments: Iterable[str | dict]) -> None:
    ...
    for a in attachments:
        if isinstance(a, str):
            attach = frappe.db.get_value("File", {"id": a}, ["file_url", "is_private"], as_dict=1)
            file_args = {
...
def mark_email_as_seen(id: str | None = None):
    frappe.request.after_response.add(lambda: _mark_email_as_seen(id))
    frappe.response.update(frappe.utils.get_imaginary_pixel_response())


def _mark_email_as_seen(id):
    try:
        update_communication_as_read(id)
    except Exception:
        frappe.log_error("Unable to mark as seen", None, "Communication", id)

    frappe.db.commit()  # nosemgrep: after_response requires explicit commit


def update_communication_as_read(id):
    if not id or not isinstance(id, str):
        return

    communication = frappe.db.get_value("Communication", id, "read_by_recipient", as_dict=True)
