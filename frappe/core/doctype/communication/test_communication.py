...
class TestCommunication(IntegrationTestCase):
    ...
    def test_circular_linking(self):
        ...
        a = frappe.get_doc("Communication", a.id)
    ...
    def test_deduplication_timeline_links(self):
        ...
        comm.add_link(link_doctype="Note", link_id=note.id, autosave=True)
        comm.add_link(link_doctype="Note", link_id=note.id, autosave=True)
    ...
    def test_contacts_attached(self):
        ...
        comm = frappe.get_doc("Communication", comm.id)
        contact_links = [x.link_id for x in comm.timeline_links]

        self.assertIn(contact_sender.id, contact_links)
        self.assertIn(contact_recipient.id, contact_links)
        self.assertIn(contact_cc.id, contact_links)
    ...
    def test_get_communication_data(self):
        ...
        comm_note_1.add_link(link_doctype="Note", link_id=note.id, autosave=True)
        ...
        comm_note_2.add_link(link_doctype="Note", link_id=note.id, autosave=True)

        comms = get_communication_data("Note", note.id, as_dict=True)

        data = [comm.id for comm in comms]
...
class TestCommunicationEmailMixin(IntegrationTestCase):
    ...
    def test_sendmail(self):
        ...
        comm.send_email()
        doc = EmailQueue.find_one_by_filters(communication=comm.id)
    ...
    def test_add_attachments_by_filename(self):
        ...
        add_attachments(comm.id, [file.id])

        attached_file_name, attached_content_hash = frappe.db.get_value(
            "File",
            {"attached_to_id": comm.id, "attached_to_doctype": comm.doctype},
    ...
    def test_add_attachments_by_file_content(self):
        ...
        add_attachments(comm.id, [{"fcontent": file_content, "fname": file_name}])
