...
class DocShare(Document):
    ...
    def get_doc(self):
        if not getattr(self, "_doc", None):
            self._doc = frappe.get_lazy_doc(self.share_doctype, self.share_id)
        return self._doc
        ...
        if self.submit and not cint(frappe.db.get_value("DocType", self.share_doctype, "is_submittable")):
            frappe.throw(
                _("Cannot share {0} with submit permission as the doctype {1} is not submittable").format(
                    frappe.bold(self.share_id), frappe.bold(self.share_doctype)
                )
            )
...
def on_doctype_update():
    ...
    frappe.db.add_index("DocShare", ["share_doctype", "share_id"])
