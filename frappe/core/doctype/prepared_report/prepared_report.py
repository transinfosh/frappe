...
class PreparedReport(Document):
    ...
    def get_prepared_data(self, with_file_name=False):
        if attachments := get_attachments(self.doctype, self.id):
            attachment = None
            ...
            attached_file = frappe.get_doc("File", attachment.id)
...
def generate_report(prepared_report):
    ...
    try:
        ...
        create_json_gz_file(result, instance.doctype, instance.id, instance.report_name)
...
def delete_prepared_reports(reports):
    ...
    for report in reports:
        prepared_report = frappe.get_doc("Prepared Report", report["id"])
