...
class UserType(Document):
    ...
    def on_trash(self):
        if self.is_standard:
            frappe.throw(_("Standard user type {0} can not be deleted.").format(frappe.bold(self.id)))
    ...
    def set_modules(self):
        ...
        modules = frappe.get_all(
            "DocType",
            filters={"id": ("in", [d.document_type for d in self.user_doctypes])},
            distinct=True,
            pluck="module",
        )
    ...
    def validate_document_type_limit(self):
        limit = frappe.conf.get("user_type_doctype_limit", {}).get(frappe.scrub(self.id))
    ...
    def update_users(self):
        for row in frappe.get_all("User", filters={"user_type": self.id}):
            user = frappe.get_cached_doc("User", row.id)
    ...
    def update_modules_in_user(self, user):
        block_modules = frappe.get_all(
            "Module Def",
            fields=["id as module"],
            filters={"id": ["not in", [d.module for d in self.user_type_modules]]},
        )
    ...
    def remove_permission_for_deleted_doctypes(self):
        ...
        for perm in frappe.get_all(
            ...
            frappe.delete_doc("Custom DocPerm", perm.id)


def add_role_permissions(doctype, role):
    id = frappe.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))

    if not id:
        id = add_permission(doctype, role, 0)
...
def get_non_standard_user_types():
    user_types = frappe.get_all(
        "User Type",
        fields=["apply_user_permission_on", "id", "user_id_field"],
        filters={"is_standard": 0},
    )

    return {d.id: [d.apply_user_permission_on, d.user_id_field] for d in user_types}
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
    ...
    filters = [
        ...
        ["DocType", "read_only", "=", 0],
        ["DocType", "id", "like", f"%{txt}%"],
    ]

    doctypes = frappe.get_all(
        "DocType",
        fields=["`tabDocType`.`id`"],
        filters=filters,
    ...
    custom_doctypes = frappe.get_all(
        "Custom Field", fields=["dt as id"], filters=custom_dt_filters, as_list=1
    )
...
def user_linked_with_permission_on_doctype(doc, user):
    ...
    if not doc.user_id_field:
        frappe.throw(_("User Id Field is mandatory in the user type {0}").format(frappe.bold(doc.id)))

    if frappe.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "id"):
        return True
    else:
    ...
    for user_type, data in user_types.items():
        ...
        if doc.get(data[1]) and (
            ...
            or not frappe.db.get_value(
                "User Permission", {"user": doc.get(data[1]), "allow": data[0], "for_value": doc.id}, "id"
            )
        ):
            perm_data = frappe.db.get_value(
                "User Permission", {"allow": doc.doctype, "for_value": doc.id}, ["id", "user"]
            )
            ...
            if not perm_data:
                ...
                add_user_permission(doc.doctype, doc.id, doc.get(data[1]))
            else:
