...
class AuditTrail(Document):
    ...
    def compare_document(self):
        self.validate()
        amended_document_ids = self.get_amended_documents()
        self.amended_docs = [frappe.get_doc(self.doctype_id, id) for id in amended_document_ids]
    ...
    def get_diff_grid(self, i, diff):
        for change in diff.changed:
            fieldname = get_field_label(change[0], doctype=self.doctype_id)
            value = change[-1]
    ...
    def get_rows_added_removed_grid(self, i, diff, key, changed_dict):
        doc_id = self.amended_docs[i].id
        changed_dict[doc_id] = {}
        for change in diff[key]:
            tablename = get_field_label(change[0], doctype=self.doctype_id)
            value_dict = filter_fields_for_gridview(change[-1])
            changed_dict[doc_id].setdefault(tablename, []).append(value_dict)

    def get_rows_updated_grid(self, i, diff):
        for change in diff.row_changed:
            table_name = get_field_label(change[0], doctype=self.doctype_id)
            index = change[1]
            self.row_changed.setdefault(table_name, {}).setdefault(index, {})
            for field in change[-1]:
                fieldname = get_field_label(field[0], doctype=self.doctype_id, child_field=change[0])
                value = field[-1]
                value_list = [""] * self.docs_to_compare
