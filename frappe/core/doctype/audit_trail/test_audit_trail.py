...
class TestAuditTrail(IntegrationTestCase):
    ...
    def test_compare_changed_fields(self):
        ...
        comparator = create_comparator_doc("Test Custom Doctype for Doc Comparator", re_amended_doc.id)
    ...
    def test_compare_rows(self):
        ...
        comparator = create_comparator_doc("Test Custom Doctype for Doc Comparator", amended_doc.id)
        documents, results = comparator.compare_document()

        results = frappe._dict(results)
        self.check_rows_updated(results.row_changed)
        self.check_rows_added(results.added[amended_doc.id])
    ...
    child_doctype = frappe.get_doc(
        {
            "doctype": "DocType",
            "module": "Core",
            "id": "Test Custom Child for Doc Comparator",
            "custom": 1,
            "istable": 1,
            "fields": [
                {
    ...
    custom_doctype = frappe.get_doc(
        {
            "doctype": "DocType",
            "module": "Core",
            "id": "Test Custom Doctype for Doc Comparator",
            "custom": 1,
            "is_submittable": 1,
            "fields": [
                {
...
def amend_document(amend_from, changed_fields, rows_updated, submit=False):
    ...
    amended_doc.amended_from = amend_from.id
...
def create_comparator_doc(doctype_id, document):
