...
class ModuleDef(Document):
    ...
    def create_modules_folder(self):
        ...
        module_path = frappe.get_app_path(self.app_name, self.id)
    ...
    def add_to_modules_txt(self):
        ...
        modules = None
        if not frappe.local.module_app.get(frappe.scrub(self.id)):
            with open(frappe.get_app_path(self.app_name, "modules.txt")) as f:
                content = f.read()
                if self.id not in content.splitlines():
                    modules = list(filter(None, content.splitlines()))
                    modules.append(self.id)
    ...
    def on_trash(self):
        ...
        if frappe.local.module_app.get(frappe.scrub(self.id)):
