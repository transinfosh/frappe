...
class Domain(Document):
        ...
        if self.data.restricted_roles:
            ...
            for role_id in self.data.restricted_roles:
                ...
                if not frappe.db.get_value("Role", role_id):
                    frappe.get_doc(doctype="Role", role_id=role_id).insert()
                    continue
    ...
    def setup_data(self, domain=None):
        ...
        self.data = frappe.get_domain_data(self.id)

    def get_domain_data(self, module):
        return frappe.get_attr(frappe.get_hooks("domains")[self.id] + ".data")
