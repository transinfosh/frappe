...
def get_user_permissions(user=None):
    ...
    def add_doc_to_perm(perm, doc_id, is_default):
        ...
        out[perm.allow].append(
            frappe._dict(
                {"doc": doc_id, "applicable_for": perm.get("applicable_for"), "is_default": is_default}
            )
        )
...
@frappe.whitelist()
def check_applicable_doc_perm(user, doctype, docid):
...
def add_user_permissions(data):
    ...
    perm_applied_docs = check_applicable_doc_perm(data.user, data.doctype, data.docid)
    exists = frappe.db.exists(
        "User Permission",
        {
            "user": data.user,
            "allow": data.doctype,
            "for_value": data.docid,
            "apply_to_all_doctypes": 1,
        },
    )
    if data.apply_to_all_doctypes == 1 and not exists:
        remove_applicable(perm_applied_docs, data.user, data.doctype, data.docid)
        insert_user_perm(
            data.user, data.doctype, data.docid, data.is_default, data.hide_descendants, apply_to_all=1
        )
        return 1
    elif len(data.applicable_doctypes) > 0 and data.apply_to_all_doctypes != 1:
        remove_apply_to_all(data.user, data.doctype, data.docid)
        update_applicable(perm_applied_docs, data.applicable_doctypes, data.user, data.doctype, data.docid)
...
def insert_user_perm(
    user, doctype, docid, is_default=0, hide_descendants=0, apply_to_all=None, applicable=None
):
    user_perm = frappe.new_doc("User Permission")
    user_perm.user = user
    user_perm.allow = doctype
    user_perm.for_value = docid
...
def remove_applicable(perm_applied_docs, user, doctype, docid):
...
def remove_apply_to_all(user, doctype, docid):
    frappe.db.delete(
        "User Permission",
        {
            "apply_to_all_doctypes": 1,
...
def update_applicable(already_applied, to_apply, user, doctype, docid):
