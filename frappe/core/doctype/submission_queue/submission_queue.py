...
class SubmissionQueue(Document):
    ...
    @property
    def queued_doc(self):
        return getattr(self, "to_be_queued_doc", frappe.get_doc(self.ref_doctype, self.ref_docid))
            ...
            frappe.publish_realtime(
                "msgprint",
                {
                    "message": message.format(
                        *message_replacements,
                        f"<a href='/app/{quote(doctype.lower().replace(' ', '-'))}/{quote(docid)}'><b>here</b></a>",
                    ),
                    "alert": True,
...
@frappe.whitelist()
def get_latest_submissions(doctype, docid):
