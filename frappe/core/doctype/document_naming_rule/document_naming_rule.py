...
class DocumentNamingRule(Document):
    ...
    def apply(self, doc):
        """
        Apply naming rules for the given document. Will set `id` if the rule is matched.
        """
        if self.conditions:
        ...
        counter = frappe.db.get_value(self.doctype, self.id, "counter", for_update=True) or 0
        naming_series = parse_naming_series(self.prefix, doc=doc)

        doc.id = naming_series + ("%0" + str(self.prefix_digits) + "d") % (counter + 1)
        frappe.db.set_value(self.doctype, self.id, "counter", counter + 1)
