...
class TestDocumentNamingRule(IntegrationTestCase):
    def test_naming_rule_by_series(self):
        ...
        todo = frappe.get_doc(doctype="ToDo", description="Is this my id " + frappe.generate_hash()).insert()
    ...
    def test_naming_rule_by_condition(self):
        ...
        try:
            ...
            self.assertEqual(todo_1.id, "test-medium-00001")
            self.assertEqual(todo_2.id, "test-low-00001")
        finally:
            naming_rule.delete()
