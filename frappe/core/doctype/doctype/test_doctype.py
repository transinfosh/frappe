...
class TestDocType(IntegrationTestCase):
    ...
    def test_validate_search_fields(self):
        ...
        doc.insert()
        self.assertEqual(doc.id, "Test Search Fields")
        ...
        test_doctype = frappe.get_doc(
            {
                ...
                "id": "Test Field Order DocType",
                "__islocal": 1,
            }
        )

        path = get_file_path(test_doctype.module, test_doctype.doctype, test_doctype.id)
    ...
    def test_cancel_link_doctype(self):
        ...
        data_doc.test_linked_doctype = data_link_doc.id
        data_doc.insert()
        data_doc.save()
        data_doc.submit()

        docs = get_submitted_linked_docs(link_doc.id, data_link_doc.id)
    ...
    def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
        ...
        data_doc_2.test_linked_doctype_a = data_link_doc_1.id
        data_doc_2.insert()
        data_doc_2.save()
        data_doc_2.submit()
        ...
        data_doc.some_fieldname = "Data1"
        data_doc.test_linked_doctype_a = data_link_doc_1.id
        data_doc.insert()
        data_doc.save()
        data_doc.submit()
    ...
    def test_autoincremented_doctype_transition(self):
        ...
        dt_data = frappe.get_doc({"doctype": dt.id, "some_fieldname": "test data"}).insert(
    ...
    def test_json_field(self):
        ...
        test_json = frappe.get_doc("Test Json Doctype", doc.id)
    ...
    def test_custom_field_deletion(self):
        ...
        doctype = new_doctype(custom=0).insert().id
    ...
    def test_delete_orphaned_doctypes(self):
        ...
        delete_controllers(doctype.id, doctype.module)
        job = frappe.enqueue(remove_orphan_doctypes)
        wait_for_completion(job)

        frappe.db.rollback()
        self.assertFalse(frappe.db.exists("DocType", doctype.id))
    ...
    def test_not_in_list_view_for_not_allowed_mandatory_field(self):
        ...
        frappe.delete_doc("DocType", doctype.id)
    ...
    def test_meta_serialization(self):
        ...
        doc = frappe.new_doc(doctype.id, some_fieldname="something").insert()
        doc.save()
        doc.submit()
        frappe.get_meta(doctype.id).as_dict()

    def test_row_compression(self):
        if frappe.db.db_type != "mariadb":
            return

        compressed_dt = new_doctype(row_format="Compressed").insert().id
        dynamic_dt = new_doctype().insert().id
...
def new_doctype(
    ...
    **kwargs,
    ...
    if not id:
        ...
        id = "Test " + "".join(random.sample(string.ascii_lowercase, 10))
