...
class DocType(Document):
    ...
    def on_update(self):
        ...
        try:
            frappe.db.updatedb(self.id, Meta(self))
    ...
    def preserve_naming_series_options_in_property_setter(self):
        ...
        if not (
            frappe.db.table_exists(self.id) and frappe.get_all(self.id, fields=["id"], limit=1, as_list=True)
        ):
            return
...
def change_id_column_type(doctype_id: str, type: str) -> None:
    ...
    args = (
        (doctype_id, "id", type, False, True)
        if (frappe.db.db_type == "postgres")
        else (doctype_id, "id", type, True)
    )
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
    ...
    try:
        ...
        if "tabModule Def" in frappe.db.get_tables() and not frappe.db.exists("Module Def", doc.module):
            m = frappe.get_doc({"doctype": "Module Def", "id": doc.module})
