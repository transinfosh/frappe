...
def get_current_index():
    ...
    current = (frappe.qb.from_(series).where(series.id == "TRANSACTLOG").for_update().select("current")).run()
    ...
    if current and current[0][0] is not None:
        ...
        frappe.db.sql("INSERT INTO `tabSeries` (id, current) VALUES ('TRANSACTLOG', 1)")
        current = 1
    return current
