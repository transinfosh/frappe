...
""",
    ...
    dict(
        id="test_invalid_namespace_method",
        script_type="DocType Event",
        doctype_event="Before Insert",
...
class TestServerScript(IntegrationTestCase):
    ...
    def test_restricted_qb(self):
        ...
        script = frappe.get_doc(
            ...
            id="test_qb_restrictions",
            script_type="API",
            api_method="test_qb_restrictions",
            allow_guest=1,
            # whitelisted update
            script=f"""
frappe.db.set_value("ToDo", "{todo.id}", "description", "safe")
""",
        )
        script.insert()
...
frappe.qb.update(todo).set(todo.description, "unsafe").where(todo.id == "{todo.id}").run()
"""
        script.save()
        ...
        script.script = f"""
todo = frappe.qb.DocType("ToDo")
frappe.qb.from_(todo).select(todo.id).where(todo.id == "{todo.id}").run()
"""
        script.save()
        script.execute_method()
    ...
    def test_server_script_rate_limiting(self):
        ...
        script2 = frappe.get_doc(
            doctype="Server Script",
            id="rate_limited_server_script2",
            script_type="API",
            enable_rate_limit=1,
            allow_guest=1,
            rate_limit_count=5,
    ...
    def test_server_script_scheduled(self):
        ...
        sync_jobs()
        self.assertTrue(frappe.db.exists("Scheduled Job Type", {"server_script": scheduled_script.id}))

        cron_job_name = frappe.db.get_value("Scheduled Job Type", {"server_script": cron_script.id})
        ...
        updated_cron_job_name = frappe.db.get_value("Scheduled Job Type", {"server_script": cron_script.id})
