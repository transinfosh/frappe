...
class Importer:
    ...
    def insert_record(self, doc):
        ...
        if not doc.id and (meta.autoname or "").lower() != "prompt":
            # id can only be set directly if autoname is prompt
            new_doc.set("id", None)
    ...
    def print_import_log(self, import_log):
        ...
        if failed_records:
            ...
            file_id = f"{self.doctype}_import_on_{frappe.utils.now()}.txt"
            print("Check {} for errors".format(os.path.join("sites", file_id)))
            text = ""
...
class ImportFile:
    ...
    def read_file(self, file_path: str):
        ...
        file_id = frappe.db.get_value("File", {"file_url": file_path})
        if file_id:
