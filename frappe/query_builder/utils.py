...
def execute_child_queries(queries, result):
    if not isinstance(result[0], dict) or not result[0].id:
        return
    parent_ids = [d.id for d in result]
    for child_query in queries:
        data = child_query.get_query(parent_ids).run(as_dict=1)
        for row in result:
            row[child_query.fieldname] = []
            for d in data:
                if str(d.parent) == str(row.id) and d.parentfield == child_query.fieldname:
