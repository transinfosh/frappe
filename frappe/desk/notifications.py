...
def get_notifications():
    ...
    for id in groups:
        count = frappe.cache.hget("notification_count:" + id, frappe.session.user)
        if count is not None:
            notification_count[id] = count
                ...
                try:
                    if isinstance(condition, dict):
                        doc_list = frappe.get_list(
                            doctype,
                            fields=["id", target_field, value_field],
                            filters=condition,
                    ...
                    for doc in doc_list:
                        ...
                        doc_target_percents[doctype][doc.id] = (
...
def clear_doctype_notifications(doc, method=None, *args, **kwargs):
    ...
    if isinstance(doc, str):
        doctype = doc  # assuming doctype id was passed directly
    else:
        doctype = doc.doctype
...
def get_notification_info():
    ...
    conditions = {}
    module_doctypes = {}
    doctype_info = dict(frappe.db.sql("""select id, module from tabDocType"""))
...
@frappe.whitelist()
@frappe.read_only()
def get_open_count(doctype: str, id: str, items=None):
    ...
    try:
        return _get_linked_document_counts(doctype, id, items)
    except Exception as e:
...
def _get_linked_document_counts(doctype: str, id: str, items=None):
    ...
    for d in items:
        ...
        if internal_link_for_doctype:
            ...
            if internal_links_data_for_d["count"]:
                ...
                try:
                    external_links_data_for_d = get_external_links(d, id, links)
    ...
    if not meta.custom:
        ...
        if hasattr(module, "get_timeline_data"):
            out["timeline_data"] = module.get_timeline_data(doctype, id)
...
def notify_mentions(ref_doctype, ref_id, content):
    if ref_doctype and ref_id and content:
        ...
        title = get_title(ref_doctype, ref_id)

        recipients = [
            frappe.db.get_value(
                "User",
                {"enabled": 1, "id": id, "user_type": "System User", "allowed_in_mentions": 1},
                "email",
            )
            for id in mentions
        ]
