...
def setup_group_by(data):
    """Add columns for aggregated values e.g. count(id)"""
...
def save_report(id, doctype, report_settings):
    """Save reports of type Report Builder from Report View"""

    if frappe.db.exists("Report", id):
    ...
    frappe.msgprint(
        _("Report {0} saved").format(frappe.bold(report.id)),
        indicator="green",
        alert=True,
    )
    return report.id
...
def delete_report(id):
    ...
    frappe.msgprint(
        _("Report {0} deleted").format(frappe.bold(report.id)),
        indicator="green",
        alert=True,
    )
...
def export_query():
    ...
    if selection := form_params.pop("selected_items", None):
        form_params["filters"] = {"id": ("in", json.loads(selection))}
...
def get_field_info(fields, doctype):
    ...
    for key in fields:
        ...
        if parenttype == doctype and fieldname == "id":
            id = fieldname
            label = _("ID", context="Label of id column in report")
            fieldtype = "Data"
            translatable = True
        else:
        ...
        field_info.append({"id": id, "label": label, "fieldtype": fieldtype, "translatable": translatable})
    ...
    for tag in tags:
        ...
        if tag["type"] not in ["Date", "Datetime"]:
            tagcount = frappe.get_list(
                doctype,
                fields=[tag["id"], "count(*)"],
                filters=[*filters, "ifnull(`{}`,'')!=''".format(tag["id"])],
                group_by=tag["id"],
                as_list=True,
            )
            ...
            if stats[tag["id"]]:
                data = [
                    ...
                    frappe.get_list(
                        doctype,
                        fields=[tag["id"], "count(*)"],
                        filters=[*filters, "({0} = '' or {0} is null)".format(tag["id"])],
                        as_list=True,
                    )[0][1],
                ]
                if data and data[1] != 0:
                    stats[tag["id"]].append(data)
        else:
            stats[tag["id"]] = tagcount
