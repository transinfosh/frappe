...
class ToDo(Document):
    ...
    def add_assign_comment(self, text, comment_type):
        if not (self.reference_type and self.reference_id):
            return

        frappe.get_doc(self.reference_type, self.reference_id).add_comment(comment_type, text)

    def delete_communication_links(self):
        # unlink todo from linked comments
        return frappe.db.delete("Communication Link", {"link_doctype": self.doctype, "link_id": self.id})
...
def on_doctype_update():
    frappe.db.add_index("ToDo", ["reference_type", "reference_id"])
