...
def get_email_header(doc, language: str | None = None):
    ...
    header_map = {
        ...
        "Assignment": _("Assignment Update on {0}", lang=language).format(docid),
        "Share": _("New Document Shared {0}", lang=language).format(docid),
    }
...
@frappe.whitelist()
def mark_as_read(docid: str):
    if frappe.flags.read_only:
        return

    if docid:
        frappe.db.set_value("Notification Log", str(docid), "read", 1, update_modified=False)
