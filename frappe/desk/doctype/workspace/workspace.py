...
class Workspace(Document):
    ...
    def on_update(self):
        ...
        if frappe.conf.developer_mode and self.public:
            if self.module:
                export_to_files(record_list=[["Workspace", self.id]], record_module=self.module)
    ...
    def get_module_wise_workspaces():
        ...
        for workspace in workspaces:
            if not workspace.module:
                continue
            module_workspaces[workspace.module].append(workspace.id)
...
@frappe.whitelist()
def update_page(id, title, icon, indicator_color, parent, public):
    public = frappe.parse_json(public)
    doc = frappe.get_doc("Workspace", id)
    ...
    if doc:
        ...
        if id != new_id:
            rename_doc("Workspace", id, new_id, force=True, ignore_permissions=True)
        ...
        if child_docs:
            for child in child_docs:
                ...
                if child.id != new_child_id:
                    rename_doc("Workspace", child.id, new_child_id, force=True, ignore_permissions=True)
