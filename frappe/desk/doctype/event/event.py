...
class Event(Document):
    ...
    def on_trash(self):
        communications = frappe.get_all(
            "Communication",
            filters={"reference_doctype": self.doctype, "reference_id": self.id},
            pluck="id",
        )
    ...
    def update_communication(self, participant: "EventParticipants", communication: "Communication"):
        ...
        communication.add_link(participant.reference_doctype, participant.reference_docid)
        communication.save(ignore_permissions=True)

    def add_participant(self, doctype, docid):
    ...
    def add_participants(self, participants):
        ...
        Args:
                participants ([Array]): Array of a dict with doctype and docid
        """
        for participant in participants:
            self.add_participant(participant["doctype"], participant["docid"])
        ...
        for participant in self.event_participants:
            ...
            if participant.reference_doctype != "Contact":
                participant_contact = get_default_contact(
                    participant.reference_doctype, participant.reference_docid
                )
            else:
                participant_contact = participant.reference_docid
...
@frappe.whitelist()
def delete_communication(event, reference_doctype, reference_docid):
    if isinstance(event, str):
        event = json.loads(event)

    deleted_participant = frappe.get_doc(reference_doctype, reference_docid)
...
def get_permission_query_conditions(user):
    ...
    if shared_events := frappe.share.get_shared("Event", user=user):
        query += f" or `tabEvent`.`id` in ({', '.join([frappe.db.escape(e) for e in shared_events])})"
    return query
