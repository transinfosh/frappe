...
class Dashboard(Document):
    ...
    def on_update(self):
        if self.is_default:
            ...
            frappe.qb.update(DashBoard).set(DashBoard.is_default, 0).where(DashBoard.id != self.id).run()
...
def get_permission_query_conditions(user):
    ...
    allowed_modules = [frappe.db.escape(module.get("id")) for module in get_modules_from_all_apps_for_user()]
...
def get_non_standard_charts_in_dashboard(dashboard):
    non_standard_charts = [doc.id for doc in frappe.get_list("Dashboard Chart", {"is_standard": 0})]
...
def get_non_standard_cards_in_dashboard(dashboard):
    non_standard_cards = [doc.id for doc in frappe.get_list("Number Card", {"is_standard": 0})]
