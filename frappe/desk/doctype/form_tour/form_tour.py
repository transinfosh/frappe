...
class FormTour(Document):
    ...
    def on_update(self):
        ...
        if frappe.conf.developer_mode and self.is_standard:
            export_to_files([["Form Tour", self.id]], self.module)
...
@frappe.whitelist()
def reset_tour(tour_id):
    for user in frappe.get_all("User", pluck="id"):
        onboarding_status = frappe.parse_json(frappe.db.get_value("User", user, "onboarding_status"))
        onboarding_status.pop(tour_id, None)
        frappe.db.set_value(
...
def get_onboarding_ui_tours():
    ...
    ui_tours = frappe.get_all("Form Tour", filters={"ui_tour": 1}, fields=["page_route", "id"])

    return [[tour.id, json.loads(tour.page_route)] for tour in ui_tours]
