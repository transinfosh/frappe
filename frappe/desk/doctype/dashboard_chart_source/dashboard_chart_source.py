...
@frappe.whitelist()
def get_config(id: str) -> str:
    doc: DashboardChartSource = frappe.get_doc("Dashboard Chart Source", id)
...
class DashboardChartSource(Document):
    ...
    def on_update(self):
        ...
        export_to_files(record_list=[[self.doctype, self.id]], record_module=self.module, create_init=True)
    ...
    def read_config(self) -> str:
        ...
        config_path = self.get_folder_path() / f"{scrub(self.id)}.js"
    ...
    def get_folder_path(self) -> Path:
        ...
        return Path(get_module_path(self.module)) / FOLDER_NAME / frappe.scrub(self.id)
