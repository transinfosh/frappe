...
def get_all_webhooks():
    ...
    webhooks_list = frappe.get_all(
        "Webhook",
        fields=["id", "condition", "webhook_docevent", "webhook_doctype", "background_jobs_queue"],
...
def flush_webhook_execution_queue():
    ...
    # deduplicate on (doc.id, webhook.id)
    # 'doc' holds the last instance values
    for execution in frappe.local._webhook_queue:
        key = (execution.webhook.get("id"), execution.doc.get("id"))
