...
class ConnectedApp(Document):
    ...
    def validate(self):
        ...
        callback_path = (
            "/api/method/frappe.integrations.doctype.connected_app.connected_app.callback/" + self.id
        )
    ...
    def initiate_web_application_flow(self, user=None, success_uri=None):
        ...
        if not token_cache:
            ...
            token_cache.connected_app = self.id
...
def callback(code=None, state=None):
    ...
    token_cache = frappe.get_doc("Token Cache", connected_app.id + "-" + frappe.session.user)
