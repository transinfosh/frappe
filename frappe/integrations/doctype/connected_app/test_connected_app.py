...
class TestConnectedApp(IntegrationTestCase):
    ...
    def tearDown(self):
        ...
        if getattr(self, "oauth_client", None):
            tokens = frappe.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.id})
            for token in tokens:
                doc = frappe.get_doc("OAuth Bearer Token", token.id)
                doc.delete()

            codes = frappe.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.id})
            for code in codes:
                doc = frappe.get_doc("OAuth Authorization Code", code.id)
                doc.delete()
