...
class OAuthWebRequestValidator(RequestValidator):
    ...
    def validate_client_id(self, client_id, request, *args, **kwargs):
        ...
        cli_id = frappe.db.get_value("OAuth Client", {"id": client_id})
        if cli_id:
    ...
    def authenticate_client_id(self, client_id, request, *args, **kwargs):
        cli_id = frappe.db.get_value("OAuth Client", client_id, "id")
        if not cli_id:
    ...
    def validate_code(self, client_id, code, client, request, *args, **kwargs):
        ...
        if code in [vcode["id"] for vcode in validcodes]:
    ...
    def save_bearer_token(self, token, request, *args, **kwargs):
        ...
        otoken.client = request.client["id"]
        try:
            otoken.user = (
                request.user
                if request.user
        ...
        return frappe.db.get_value("OAuth Client", request.client["id"], "default_redirect_uri")
        ...
        if id_token_hint:
            try:
                ...
                if payload.get("sub") and client_id and client_secret:
                    ...
                    if verified_payload:
                        return user.id == frappe.session.user
    ...
    return frappe._dict(
        {
            "sub": frappe.db.get_value(
                "User Social Login",
                {"parent": user.id, "provider": "frappe"},
                "userid",
            ),
            "id": " ".join(filter(None, [user.first_name, user.last_name])),
            "given_name": user.first_name,
            "family_name": user.last_name,
            "email": user.email,
            "picture": picture,
            "roles": frappe.get_roles(user.id),
