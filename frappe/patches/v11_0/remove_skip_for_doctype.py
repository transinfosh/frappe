...
def execute():
    ...
    for user_permission in frappe.get_all("User Permission", fields=["*"]):
        ...
        if skip_for_doctype:
            ...
            user_permissions_to_delete.append(user_permission.id)
            user_permission.id = None
            ...
            frappe.db.set_value("User Permission", user_permission.id, "apply_to_all_doctypes", 1)
    ...
    if user_permissions_to_delete:
        frappe.db.delete("User Permission", filters=(Field("id").isin(tuple(user_permissions_to_delete))))
