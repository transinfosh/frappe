...
def execute():
    ...
    for _field in rating_fields + custom_rating_fields:
        doctype_id = _field.get("parent") or _field.get("dt")
        doctype = DocType(doctype_id)
        field = _field.fieldname

        # TODO: Add postgres support (for the check)
        if (
            frappe.conf.db_type == "mariadb"
            and frappe.db.get_column_type(doctype_id, field) == RATING_FIELD_TYPE
        ):
            continue
        ...
        frappe.db.change_column_type(doctype_id, column=field, type=RATING_FIELD_TYPE, nullable=True)
