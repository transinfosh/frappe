...
def run_patch():
    ...
    users = frappe.get_all("User", fields=["*"], filters={"id": ("not in", ["Administrator", "Guest"])})

    for user in users:
        idx = 0
        if user.frappe_userid:
            insert_user_social_login(user.id, user.modified_by, "frappe", idx, userid=user.frappe_userid)
            idx += 1

        if user.fb_userid or user.fb_username:
            insert_user_social_login(
                user.id, user.modified_by, "facebook", idx, userid=user.fb_userid, username=user.fb_username
            )
            idx += 1
        ...
        if user.google_userid:
            insert_user_social_login(user.id, user.modified_by, "google", idx, userid=user.google_userid)
            idx += 1
