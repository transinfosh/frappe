...
def setup_incoming_email_port_in_email_domains():
    email_domains = frappe.get_all("Email Domain", ["incoming_port", "use_imap", "use_ssl", "id"])
    for domain in email_domains:
        if not domain.incoming_port:
            incoming_port = get_port(domain)
            frappe.db.set_value(
                "Email Domain", domain.id, "incoming_port", incoming_port, update_modified=False
            )

            # update incoming email port in all
            frappe.db.sql(
                """update `tabEmail Account` set incoming_port=%s where domain = %s""",
                (domain.incoming_port, domain.id),
            )


def setup_incoming_email_port_in_email_accounts():
    email_accounts = frappe.get_all(
        "Email Account", ["incoming_port", "use_imap", "use_ssl", "id", "enable_incoming"]
    )
    ...
    for account in email_accounts:
        if account.enable_incoming and not account.incoming_port:
            ...
            frappe.db.set_value(
                "Email Account", account.id, "incoming_port", incoming_port, update_modified=False
            )
