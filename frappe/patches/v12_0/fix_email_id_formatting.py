...
def fix_communications():
    for communication in frappe.db.sql(
        ...
        frappe.db.sql(
            ...
            where id =%s """,
            (communication["recipients"], communication["cc"], communication["bcc"], communication["id"]),
        )
...
def fix_show_as_cc_email_queue():
    for queue in frappe.get_all(
        ...
        frappe.db.set_value("Email Queue", queue["id"], "show_as_cc", format_email_id(queue["show_as_cc"]))
...
def fix_email_queue_recipients():
    for recipient in frappe.db.sql(
        ...
        frappe.db.set_value(
            "Email Queue Recipient", recipient["id"], "recipient", format_email_id(recipient["recipient"])
        )
