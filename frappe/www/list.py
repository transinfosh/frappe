...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
    ...
    for doc in raw_result:
        ...
        if not list_context.get_list and not isinstance(new_context.doc, Document):
            new_context.doc = frappe.get_doc(doc.doctype, doc.id)
...
def get_list_data(
    ...
    list_context.title_field = getattr(controller, "website", {}).get(
        "page_title_field", meta.title_field or "id"
    )
...
def set_route(context):
    ...
    if context.web_form_name:
        context.route = f"{context.pathname}?id={quoted(context.doc.id)}"
    elif context.doc and getattr(context.doc, "route", None):
        context.route = context.doc.route
    else:
        context.route = f"{context.pathname or quoted(context.doc.doctype)}/{quoted(context.doc.id)}"
    ...
    if txt:
        if meta.search_fields:
            or_filters.extend(
                ...
                if f == "id" or meta.get_field(f).fieldtype in ("Data", "Text", "Small Text", "Text Editor")
            )
        else:
            if isinstance(filters, dict):
                filters["id"] = ("like", "%" + txt + "%")
            else:
                filters.append([doctype, "id", "like", "%" + txt + "%"])
