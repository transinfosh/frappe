...
class UserPermissions:
    ...
    def setup_user(self):
        def get_user_doc():
            user = None
            try:
                user = frappe.get_doc("User", self.id).as_dict()
    ...
    def build_permissions(self):
        ...
        self.shared = frappe.get_all(
            "DocShare", {"user": self.id, "read": 1}, distinct=True, pluck="share_doctype"
        )
        ...
        if "System Manager" in self.get_roles():
            self.can_import += frappe.get_all("DocType", {"allow_import": 1}, pluck="id")
    ...
    def get_defaults(self):
        ...
        self.defaults = frappe.defaults.get_defaults(self.id)
...
def get_system_managers(only_id: bool = False) -> list[str]:
    ...
    if only_id:
        ...
        fields = [User.full_name, User.id]

    system_managers = (
        frappe.qb.from_(User)
        .join(HasRole)
        .on(HasRole.parent == User.id)
        .where(
            (HasRole.parenttype == "User")
            & (User.enabled == 1)
            & (HasRole.role == "System Manager")
            & (User.docstatus < 2)
            & (User.id.notin(frappe.STANDARD_USERS))
        )
        .select(*fields)
    ...
    if only_id:
        return [p.id for p in system_managers]
    else:
        return [formataddr((p.full_name, p.id)) for p in system_managers]
...
def add_system_manager(
    ...
    roles = frappe.get_all(
        "Role",
        fields=["id"],
        filters={"id": ["not in", AUTOMATIC_ROLES]},
    )
