...
def read_csv_content_from_attached_file(doc):
    fileid = frappe.get_all(
        "File",
        fields=["id"],
        filters={"attached_to_doctype": doc.doctype, "attached_to_id": doc.id},
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
    ...
    if d.get("id") and frappe.db.exists(doctype, d["id"]):
        if overwrite:
            ...
            if d.get("docstatus") == 1:
                ...
                doc.save()
            return "Updated row (#%d) %s" % (row_idx + 1, getlink(doctype, d["id"]))
        else:
            return "Ignored row (#%d) %s (exists)" % (row_idx + 1, getlink(doctype, d["id"]))
    else:
        doc = frappe.get_doc(d)
        doc.flags.ignore_links = ignore_links
        doc.insert()
        ...
        return "Inserted row (#%d) %s" % (row_idx + 1, getlink(doctype, doc.get("id")))


def getlink(doctype, id):
    return '<a href="/app/Form/{doctype}/{id}">{id}</a>'.format(**locals())
