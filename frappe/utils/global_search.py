...
def get_doctypes_with_global_search(with_child_tables=True):
    ...
    def _get():
        ...
        for d in frappe.get_all("DocType", fields=["id", "module"], filters=filters):
            meta = frappe.get_meta(d.id)
...
def rebuild_for_doctype(doctype):
    ...
    for doc in all_records:
        ...
        # get children data
        for child_doctype, records in all_children.get(doc.id, {}).items():
        ...
        if content:
            ...
            try:
                if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
                    d = frappe.get_doc(doctype, doc.id)
...
def insert_values_for_multiple_docs(all_contents):
    values = [
        "({doctype}, {id}, {content}, {published}, {title}, {route})".format(**content)
        for content in all_contents
    ]
    batch_size = 50000
    for i in range(0, len(values), batch_size):
        batch_values = values[i : i + batch_size]
        # ignoring duplicate keys for doctype_id
        frappe.db.multisql(
            {
                ...
                (doctype, id, content, published, title, route)
                VALUES {}
                ON CONFLICT("id", "doctype") DO NOTHING""".format(", ".join(batch_values)),
...
def _get_deduped_search_item_values(items):
    ...
    for item in items:
        ...
        key = (item_dict["doctype"], item_dict["id"])
...
def sync_values(values: list):
    ...
    query = frappe.qb.into(GlobalSearch).columns(["doctype", "id", *conflict_fields]).insert(*values)

    if frappe.db.db_type == "postgres":
        query = query.on_conflict(GlobalSearch.doctype, GlobalSearch.id)
...
def sync_value(value: dict):
    ...
    :param value: dict of { doctype, id, content, published, title, route }
    """
...
def delete_for_document(doc):
    ...
    """
    frappe.db.delete("__global_search", {"doctype": doc.doctype, "id": doc.id})
...
def search(text, start=0, limit=20, doctype=""):
    ...
    for word in set(text.split("&")):
        ...
        query = (
            ...
            .select(global_search.doctype, global_search.id, global_search.content, rank.as_("rank"))
            .where(rank)
