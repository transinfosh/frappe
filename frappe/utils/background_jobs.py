...
def get_queues_timeout() -> dict[str, int]:
    """
    Method returning a mapping of queue id to timeout for that queue

    :return: Dictionary of queue id to timeout
    """
...
def run_doc_method(doctype, id, doc_method, **kwargs):
    getattr(frappe.get_doc(doctype, id), doc_method)(**kwargs)
...
def get_worker_id(queue):
    """When limiting worker to a specific queue, also append queue id to default worker id"""
    id = None

    if queue:
        # hostname.pid is the default worker name
        id = f"{uuid4().hex}.{socket.gethostname()}.{os.getpid()}.{queue}"
