...
def update_nsm(doc):
    ...
    frappe.db.set_value(doc.doctype, doc.id, old_parent_field, parent or "", update_modified=False)
...
def update_add_node(doc, parent, parent_field):
    ...
    if parent:
        left, right = frappe.db.get_value(doctype, {"id": parent}, ["lft", "rgt"], for_update=True)
        validate_loop(doc.doctype, doc.id, left, right)
    else:  # root
        right = (
    ...
    frappe.qb.update(Table).set(Table.lft, right).set(Table.rgt, right + 1).where(Table.id == id).run()
    return right
...
def update_move_node(doc: Document, parent_field: str):
    ...
    if parent:
        ...
        validate_loop(doc.doctype, doc.id, new_parent.lft, new_parent.rgt)
    ...
    if parent:
        ...
        # set parent lft, rgt
        frappe.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.id == parent).run()
...
def validate_loop(doctype, id, lft, rgt):
    """check if item not an ancestor (loop)"""
    if id in frappe.get_all(doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="id"):
        frappe.throw(_("Item cannot be added to its own descendants"), NestedSetRecursionError)


def remove_subtree(doctype: str, id: str, throw=True):
    ...
    lft, rgt = frappe.db.get_value(doctype, id, ["lft", "rgt"])
...
class NestedSet(Document):
    ...
    def validate_if_child_exists(self):
        has_children = frappe.db.count(self.doctype, filters={self.nsm_parent_field: self.id})
        if has_children:
            frappe.throw(
                _("Cannot delete {0} as it has child nodes").format(self.id), NestedSetChildExistsError
            )
    ...
    def validate_ledger(self, group_identifier="is_group"):
        if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
            if frappe.get_all(self.doctype, {self.nsm_parent_field: self.id, "docstatus": ("!=", 2)}):
                frappe.throw(
                    _("{0} {1} cannot be a leaf node as it has children").format(_(self.doctype), self.id)
                )

    def get_ancestors(self):
        return get_ancestors_of(self.doctype, self.id)

    def get_parent(self) -> "NestedSet":
        """Return the parent Document."""
        parent_id = self.get(self.nsm_parent_field)
        if parent_id:
            return frappe.get_doc(self.doctype, parent_id)

    def get_children(self) -> Iterator["NestedSet"]:
        """Return a generator that yields child Documents."""
        child_ids = frappe.get_list(self.doctype, filters={self.nsm_parent_field: self.id}, pluck="id")
        for id in child_ids:
            yield frappe.get_doc(self.doctype, id)
...
def get_root_of(doctype):
    ...
    result = frappe.qb.from_(t1).select(t1.id).where((node_query == 0) & (t1.rgt > t1.lft)).run()

    return result[0][0] if result else None


def get_ancestors_of(doctype, id, order_by="lft desc", limit=None):
...
def get_descendants_of(doctype, id, order_by="lft desc", limit=None, ignore_permissions=False):
