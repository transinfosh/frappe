...
def parse_addr(email_string):
    ...
    """
    id, email = parseaddr(email_string)
    if check_format(email):
        id = get_id_from_email_string(email_string, email, id)
        return (id, email)
    else:
...
def get_id_from_email_string(email_string, email_id, id):
    id = email_string.replace(email_id, "")
    id = EMAIL_NAME_PATTERN.sub("", id).strip()
    if not id:
        id = email_id
    return id
...
def get_site_info():
    ...
    users = frappe.get_all(
        "User",
        filters={"user_type": "System User", "id": ("not in", frappe.STANDARD_USERS)},
        fields=["id", "enabled", "last_login", "last_active", "language", "time_zone"],
    )
    system_managers = get_system_managers(only_id=True)
    for u in users:
        # tag system managers
        u.is_system_manager = 1 if u.id in system_managers else 0
        u.full_name = get_fullname(u.id)
        u.email = u.id
        del u["id"]
...
def get_db_count(*args):
    ...
    Parameters:
            *args: Variable length argument list of doctype ids whose doc count you need
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
    ...
    missing_info = frappe.get_all(
        "User",
        {"id": ("in", missing_users)},
        ["full_name", "user_image", "id", "email", "time_zone"],
    )

    for info in missing_info:
        user_info.setdefault(info.id, frappe._dict()).update(
            fullname=info.full_name or info.id,
