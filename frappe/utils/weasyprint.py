...
@frappe.whitelist()
def download_pdf(doctype, id, print_format, letterhead=None):
...
def get_html(doctype, id, print_format, letterhead=None):
    doc = frappe.get_doc(doctype, id)
    doc.check_permission("print")
    generator = PrintFormatGenerator(print_format, doc, letterhead)
        ...
        for section in layout["sections"]:
            for column in section["columns"]:
                for df in column["fields"]:
                    ...
                    renderer_id = fieldtype.replace(" ", "")
                    df["renderer"] = renderers.get(fieldtype) or renderer_id
                    df["section"] = section
        return layout
