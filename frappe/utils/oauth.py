...
def get_oauth2_providers() -> dict[str, dict]:
    ...
    for provider in providers:
        ...
        if provider.auth_url_data:
            out[provider.id]["auth_url_data"] = json.loads(provider.auth_url_data)

        if provider.api_endpoint_args:
            out[provider.id]["api_endpoint_args"] = json.loads(provider.api_endpoint_args)
...
def get_first_name(data: dict) -> str:
    return data.get("first_name") or data.get("given_name") or data.get("id")
