    ...
    for a in attachments:
        if isinstance(a, str):
            attach = frappe.db.get_value(
                "File", {"id": a}, ["file_name", "file_url", "is_private"], as_dict=1
            )
            # save attachments to new doc
            f = save_url(
                attach.file_url, attach.file_name, doctype, id, "Home/Attachments", attach.is_private
            )
            files.append(f)
