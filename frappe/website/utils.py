...
def get_comment_list(doctype, id):
    comments = frappe.get_all(
        "Comment",
        fields=["id", "creation", "owner", "comment_email", "comment_by", "content"],
        filters=dict(
...
def cleanup_page_id(title: str) -> str:
    ...
    id = title.lower()
    id = CLEANUP_PATTERN_1.sub("", id)
    id = CLEANUP_PATTERN_2.sub("-", id)
    id = "-".join(id.split())
    # replace repeating hyphens
    id = CLEANUP_PATTERN_3.sub(r"\1", id)
    return id[:140]
        ...
        def _build():
            ...
            for route, children in children_map.items():
                ...
                if page_info.index or ("index" in page_info.template):
                    new_children = []
                    page_info.extn = ""
                    for id in page_info.index or []:
                        child_route = page_info.route + "/" + id
