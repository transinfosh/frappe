...
class WebPage(WebsiteGenerator):
    ...
    def get_context(self, context):
        ...
        if self.context_script:
            ...
            safe_exec(self.context_script, None, _locals, script_filename=f"web page {self.id}")
    ...
    def set_metatags(self, context):
        if not context.metatags:
            context.metatags = {
                "id": self.meta_title or self.title,
...
def check_publish_status():
    ...
    web_pages = frappe.get_all("Web Page", fields=["id", "published", "start_date", "end_date"])
