...
class TestWebForm(IntegrationTestCase):
    ...
    def test_accept(self):
        ...
        self.event_id = frappe.db.get_value("Event", {"subject": "_Test Event Web Form"})
    ...
    def test_edit(self):
        ...
        self.assertNotEqual(
            frappe.db.get_value("Event", self.event_id, "description"), doc.get("description")
        )

        accept("manage-events", json.dumps(doc))

        self.assertEqual(frappe.db.get_value("Event", self.event_id, "description"), doc.get("description"))
