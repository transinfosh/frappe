...
class PersonalDataDeletionRequest(Document):
    ...
    def generate_url_for_confirmation(self):
        params = {"email": self.email, "id": self.id, "host_name": frappe.local.site}
    ...
    def rename_documents(self, doctype):
        ...
        for i, id in enumerate(
            ...
            frappe.rename_doc(doctype["doctype"], id, new_id(self.anon, i + 1), force=True, show_alert=False)
    ...
    def redact_doc(self, doc, ref):
        ...
        if ref.get("rename") and doc["id"] != self.anon:
            frappe.rename_doc(ref["doctype"], doc["id"], self.anon, force=True, show_alert=False)
...
def process_data_deletion_request():
    ...
    requests = frappe.get_all(
        "Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="id"
    )
