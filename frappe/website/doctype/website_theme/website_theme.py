...
class WebsiteTheme(Document):
    ...
    def export_doc(self):
        """Export to standard folder `[module]/website_theme/[id]/[id].json`."""
        from frappe.modules.export_file import export_to_files

        export_to_files(record_list=[["Website Theme", self.id]], create_init=True)

    def clear_cache_if_current_theme(self):
        if frappe.flags.in_install == "frappe":
            return
        website_settings = frappe.get_doc("Website Settings", "Website Settings")
        if getattr(website_settings, "website_theme", None) == self.id:
    ...
    def generate_bootstrap_theme(self):
        ...
        file_name = frappe.scrub(self.id) + "_" + suffix + ".css"
    ...
    def set_as_default(self):
        ...
        website_settings.website_theme = self.id
    ...
    def get_apps(self):
        ...
        apps = get_versions()
        return [{"id": app, "title": values["title"]} for app, values in apps.items()]
