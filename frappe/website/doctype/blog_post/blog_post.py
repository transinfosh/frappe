...
class BlogPost(WebsiteGenerator):
    ...
    def load_comments(self, context):
        context.comment_list = get_comment_list(self.doctype, self.id)
...
def get_list_context(context=None):
    ...
    category_id = frappe.utils.escape_html(
        frappe.local.form_dict.blog_category or frappe.local.form_dict.category
    )
    if category_id:
        category = frappe.get_doc("Blog Category", category_id)
        ...
        blogger = frappe.db.get_value("Blogger", {"id": frappe.local.form_dict.blogger}, "full_name")
    ...
    if list_context.sub_title:
        list_context.parents = [{"id": _("Home"), "route": "/"}, {"id": "Blog", "route": "/blog"}]
    else:
        list_context.parents = [{"id": _("Home"), "route": "/"}]

    if blog_settings.browse_by_category:
        list_context.blog_categories = get_blog_categories()

    list_context.metatags = {
        "id": list_context.blog_title,
...
def get_blog_categories():
    ...
    return (
        ...
        .select(category.id, category.route, category.title)
        .where(
            (category.published == 1)
            & ExistsCriterion(
                frappe.qb.from_(post)
                .select("id")
                .where((post.published == 1) & (post.blog_category == category.id))
            )
        )
