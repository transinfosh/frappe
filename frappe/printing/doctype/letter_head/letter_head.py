...
<img src="{self.get(field)}" alt="{self.get("id")}"
{dimension}="{dimension_value}" style="{dimension}: {dimension_value}px;">
    ...
    def set_as_default(self):
        ...
        if self.is_default:
            frappe.db.set_value("Letter Head", {"id": ["!=", self.id]}, "is_default", 0)

            set_default("letter_head", self.id)

            # update control panel - so it loads new letter directly
            frappe.db.set_default("default_letter_head_content", self.content)
        else:
            frappe.defaults.clear_default("letter_head", self.id)
