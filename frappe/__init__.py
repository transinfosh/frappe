...
def set_value(doctype, docid, fieldname, value=None):
    """Set document value. Calls `frappe.client.set_value`"""
    import frappe.client

    return frappe.client.set_value(doctype, docid, fieldname, value)
...
def get_desk_link(doctype, id, show_title_with_id=False):
    meta = get_meta(doctype)
    title = get_value(doctype, id, meta.get_title_field())

    if show_title_with_id and id != title:
        html = '<a href="/app/Form/{doctype}/{id}" style="font-weight: bold;">{doctype_local} {id}: {title_local}</a>'
    else:
        html = (
            '<a href="/app/Form/{doctype}/{id}" style="font-weight: bold;">{doctype_local} {title_local}</a>'
        )
