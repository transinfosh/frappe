...
class TestAutoAssign(IntegrationTestCase):
    ...
    def test_round_robin(self):
        ...
        self.assertEqual(
            frappe.db.get_value(
                "ToDo",
                dict(reference_type=TEST_DOCTYPE, reference_id=record.id, status="Open"),
                "allocated_to",
            ),
            "test@example.com",
        )
    ...
    def test_assingment_on_guest_submissions(self):
        ...
        self.assertTrue(
            frappe.db.get_value(
                "ToDo",
                {"reference_type": TEST_DOCTYPE, "reference_id": doc.id, "status": "Open"},
                "allocated_to",
            ),
        )
    ...
    def test_based_on_field(self):
        ...
        for test_user in ("test1@example.com", "test2@example.com"):
            ...
            self.assertEqual(
                frappe.db.get_value(
                    "ToDo",
                    dict(reference_type=TEST_DOCTYPE, reference_id=note.id, status="Open"),
                    "owner",
                ),
                test_user,
            )
    ...
    def test_clear_assignment(self):
        ...
        todo = frappe.get_list(
            "ToDo", dict(reference_type=TEST_DOCTYPE, reference_id=note.id, status="Open"), limit=1
        )[0]

        todo = frappe.get_doc("ToDo", todo["id"])
    ...
    def test_assignment_rule_condition(self):
        ...
        note1_todo = frappe.get_all(
            "ToDo", filters=dict(reference_type=TEST_DOCTYPE, reference_id=note1.id, status="Open")
        )[0]

        note1_todo_doc = frappe.get_doc("ToDo", note1_todo.id)
        ...
        note2_todo = frappe.get_all(
            "ToDo",
            filters=dict(reference_type=TEST_DOCTYPE, reference_id=note2.id, status="Open"),
            fields=["id", "date"],
        )[0]
    ...
    def test_submittable_assignment(self):
        ...
        assignment_rule.id = f"For {submittable_doctype}"
