...
def _try_create(record, reset=False, commit=False) -> tuple["Document", bool]:
    ...
    if frappe.db.exists(d.doctype, d.id) and not reset:
        ...
        return frappe.get_doc(d.doctype, d.id), False
        ...
        if self.log_file.exists():
            with self.log_file.open() as f:
                for line in f:
                    ...
                    try:
                        for r in records:
                            log.setdefault(index_doctype, []).append(frappe.get_doc(r["doctype"], r["id"]))
                    except frappe.DoesNotExistError as e:
                        raise ValueError(
                            f"Global test record '{r['id']}' ({r['doctype']}) had been deleted resulting in inconsistent global state."
                        ) from e
        return log
...
def make_test_records_for_doctype(doctype, force=False, commit=False):
    ...
    return list(r.id for tag, r in _generate_records_for(doctype, reset=force, commit=commit))
...
def make_test_objects(doctype=None, test_records=None, reset=False, commit=False):
    ...
    return list(r.id for tag, r in _sync_records(doctype, test_records, reset, commit))
