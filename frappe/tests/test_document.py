...
class TestDocumentWebView(IntegrationTestCase):
    ...
    def test_base_class_set_correctly_on_has_web_view_change(self):
        ...
        doc_path = Path(get_doc_path(test_doctype.module, test_doctype.doctype, test_doctype.id))
        controller_file_path = doc_path / f"{scrub(test_doctype.id)}.py"
    ...
    def test_bulk_inserts(self):
        ...
        all_child_docs = set(
            frappe.get_all(
                child_doctype, filters={"parenttype": doctype, "parentfield": child_field}, pluck="id"
            )
        )
