...
def two_factor_is_enabled_for_(user):
    ...
    no_of_users = frappe.db.count(
        role_doctype,
        filters=((role_doctype.two_factor_auth == 1) & (role_doctype.id.isin(roles))),
    )
...
def delete_qrimage(user, check_expiry=False):
    ...
    user_barcodes = frappe.get_all(
        "File", {"attached_to_doctype": "User", "attached_to_id": user, "folder": "Home/Barcodes"}
    )

    for barcode in user_barcodes:
        if check_expiry and not should_remove_barcode_image(barcode):
            continue
        barcode = frappe.get_doc("File", barcode.id)
        frappe.delete_doc("File", barcode.id, ignore_permissions=True)
...
def delete_all_barcodes_for_users():
    ...
    for user in users:
        if not two_factor_is_enabled(user=user.id):
            continue
        delete_qrimage(user.id, check_expiry=True)
