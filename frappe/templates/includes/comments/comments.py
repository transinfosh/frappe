...
@rate_limit(key="reference_id", limit=get_comment_limit, seconds=60 * 60)
def add_comment(comment, comment_email, comment_by, reference_doctype, reference_id, route):
    ...
    doc = frappe.get_doc(reference_doctype, reference_id)
    ...
    if doc.get("route"):
        url = f"{frappe.utils.get_request_site_address()}/{doc.route}#{comment.id}"
    else:
        url = f"{frappe.utils.get_request_site_address()}/app/{scrub(doc.doctype)}/{doc.id}#comment-{comment.id}"
