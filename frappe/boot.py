...
def get_bootinfo():
    ...
    bootinfo.all_domains = [d.get("id") for d in frappe.get_all("Domain")]
    add_layouts(bootinfo)

    bootinfo.module_app = frappe.local.module_app
    bootinfo.single_types = [d.id for d in frappe.get_all("DocType", {"issingle": 1})]
...
def get_letter_heads():
    ...
    for letter_head in frappe.get_list("Letter Head", fields=["id", "content", "footer"]):
        letter_heads.setdefault(letter_head.id, {"header": letter_head.content, "footer": letter_head.footer})
...
def load_desktop_data(bootinfo):
    ...
    for app_name in frappe.get_installed_apps():
        ...
        bootinfo.app_data.append(
            dict(
                app_name=app_info.get("id") or app_name,
...
def get_user_pages_or_reports(parent, cache=False):
    ...
    if is_report:
        columns = (report.id.as_("title"), report.ref_doctype, report.report_type)
    else:
    ...
    pages_with_custom_roles = (
        ...
        .from_(hasRole)
        .from_(parentTable)
        .select(customRole[parent.lower()].as_("id"), customRole.modified, customRole.ref_doctype, *columns)
        .where(
            (hasRole.parent == customRole.id)
            & (parentTable.id == customRole[parent.lower()])
    ...
    for p in pages_with_custom_roles:
        has_role[p.id] = {"modified": p.modified, "title": p.title, "ref_doctype": p.ref_doctype}
    ...
    pages_with_standard_roles = (
        ...
        .from_(parentTable)
        .select(parentTable.id.as_("id"), parentTable.modified, *columns)
        .where((hasRole.role.isin(roles)) & (hasRole.parent == parentTable.id) & (parentTable.id.notin(subq)))
        .distinct()
    )
    ...
    for p in pages_with_standard_roles:
        if p.id not in has_role:
            has_role[p.id] = {"modified": p.modified, "title": p.title}
            if parent == "Report":
                has_role[p.id].update({"ref_doctype": p.ref_doctype})

    no_of_roles = SubQuery(
        frappe.qb.from_(hasRole).select(Count("*")).where(hasRole.parent == parentTable.id)
    )

    # pages and reports with no role are allowed
    rows_with_no_roles = (
        frappe.qb.from_(parentTable)
        .select(parentTable.id, parentTable.modified, *columns)
        .where(no_of_roles == 0)
    ).run(as_dict=True)

    for r in rows_with_no_roles:
        if r.id not in has_role:
            has_role[r.id] = {"modified": r.modified, "title": r.title}
            if is_report:
                has_role[r.id] |= {"ref_doctype": r.ref_doctype}
    ...
    if is_report:
        ...
        reports = frappe.get_list(
            "Report",
            fields=["id", "report_type"],
            filters={"id": ("in", has_role.keys())},
            ignore_ifnull=True,
        )
        for report in reports:
            has_role[report.id]["report_type"] = report.report_type

        non_permitted_reports = set(has_role.keys()) - {r.id for r in reports}
...
def get_link_preview_doctypes():
    ...
    link_preview_doctypes = [d.id for d in frappe.get_all("DocType", {"show_preview_popup": 1})]
...
def add_layouts(bootinfo):
    ...
    bootinfo.doctype_layouts = frappe.get_all("DocType Layout", ["id", "route", "document_type"])
...
def get_link_title_doctypes():
    ...
    return [d.id for d in dts + custom_dts if d]
