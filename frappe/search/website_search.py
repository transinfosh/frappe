...
def slugs_with_web_view(_items_to_index):
    ...
    fields = ["id", "is_published_field", "website_search_field"]
    ...
    for doctype in doctype_with_web_views:
        if doctype.is_published_field:
            ...
            if doctype.website_search_field:
                docs = frappe.get_all(doctype.id, filters=filters, fields=[*fields, "title"])
                for doc in docs:
                ...
                docs = frappe.get_all(doctype.id, filters=filters, fields=fields)
...
def update_index_for_path(path):
    ...
    return ws.update_index_by_id(path)
