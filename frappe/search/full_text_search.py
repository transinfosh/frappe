...
class FullTextSearch:
    ...
    def get_schema(self):
        return Schema(id=ID(stored=True), content=TEXT(stored=True))
    ...
    def update_index_by_id(self, doc_id):
        """Wraps `update_index` method, gets the document from id
        ...
        Args:
                ...
                doc_id (str): id of the document to be updated
        """
        document = self.get_document_to_index(doc_id)
        if document:
            self.update_index(document)

    def remove_document_from_index(self, doc_id):
        """Remove document from search index

        Args:
                self (object): FullTextSearch Instance
                doc_id (str): id of the document to be removed
        """
        if not doc_id:
            return
        ...
        with ix.searcher():
            ...
            writer.delete_by_term(self.id, doc_id)
