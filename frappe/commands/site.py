...
@pass_context
def reload_doc(context: CliCtxObj, module, doctype, docid):
    "Reload schema for a DocType"
    for site in context.sites:
        try:
            frappe.init(site)
            frappe.connect()
            frappe.reload_doc(module, doctype, docid, force=context.force)
            frappe.db.commit()
        finally:
            frappe.destroy()
    if not context.sites:
...
@pass_context
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docid, after_commit):
