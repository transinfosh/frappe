...
def build_missing_files():
    ...
    if assets_json:
        ...
        if missing_assets:
            ...
            files_to_build = ["frappe/" + id for id in missing_assets]
...
def setup_assets(assets_archive):
    ...
    with tarfile.open(assets_archive) as tar:
        for file in tar:
            if not file.isdir():
                dest = "." + file.id.replace("./frappe-bench/sites", "")
...
def symlink(target, link_id, overwrite=False):
    """
    Create a symbolic link named link_id pointing to target.
    If link_id exists then FileExistsError is raised, unless overwrite=True.
    ...
    if not overwrite:
        return os.symlink(target, link_id)

    # Create link to target with temporary filename
    while True:
        temp_link_id = f"tmp{frappe.generate_hash()}"

        # os.* functions mimic as closely as possible system functions
        # The POSIX symlink() returns EEXIST if link_id already exists
        # https://pubs.opengroup.org/onlinepubs/9699919799/functions/symlink.html
        try:
            os.symlink(target, temp_link_id)
            break
        except FileExistsError:
            pass

    # Replace link_id with temp_link_id
    try:
        # Pre-empt os.replace on a directory with a nicer message
        if os.path.isdir(link_id):
            raise IsADirectoryError(f"Cannot symlink over existing directory: '{link_id}'")
        try:
            shutil.move(temp_link_id, link_id)
        except AttributeError:
            os.renames(temp_link_id, link_id)
    except Exception:
        if os.path.islink(temp_link_id):
            os.remove(temp_link_id)
        raise
