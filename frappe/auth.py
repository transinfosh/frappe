...
class LoginManager:
    ...
    def authenticate(self, user: str | None = None, pwd: str | None = None):
        ...
        ignore_tracker = should_run_2fa(user.id) and ("otp" in frappe.form_dict)
        user_tracker = None if ignore_tracker else get_login_attempt_tracker(user.id)

        if not user.is_authenticated:
            user_tracker and user_tracker.add_failure_attempt()
            ip_tracker and ip_tracker.add_failure_attempt()
            self.fail("Invalid login credentials", user=user.id)
        elif not (user.id == "Administrator" or user.enabled):
            user_tracker and user_tracker.add_failure_attempt()
            ip_tracker and ip_tracker.add_failure_attempt()
            self.fail("User disabled or missing", user=user.id)
        else:
...
def validate_api_key_secret(api_key, api_secret, frappe_authorization_source=None):
    ...
    docid = frappe.db.get_value(
        doctype=doctype, filters={"api_key": api_key, "enabled": True}, fieldname=["id"]
    )
    if not docid:
        raise frappe.AuthenticationError
    form_dict = frappe.local.form_dict
    doc_secret = get_decrypted_password(doctype, docid, fieldname="api_secret")
    if api_secret == doc_secret:
        if doctype == "User":
            user = frappe.db.get_value(doctype="User", filters={"api_key": api_key}, fieldname=["id"])
        else:
            user = frappe.db.get_value(doctype, docid, "user")
