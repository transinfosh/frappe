...
frappe.ui.form.ControlLink = class ControlLink extends frappe.ui.form.ControlData {
    ...
    async get_options_name() {
        if (!this.df.options) {
            return "";
        }
        let result = await frappe.db.get_value("DocType", this.df.options, "name");
                    ...
                    callback: async function (r) {
                        ...
                        if (!me.df.only_select) {
                            if (frappe.model.can_create(doctype)) {
                                let label_name = __(await me.get_options_name());
                                // new item
                                r.message.push({
                                    html:
                                        "<span class='link-option'>" +
                                        "<i class='fa fa-plus' style='margin-right: 5px;'></i> " +
                                        __("Create a new {0}", [label_name]) +
                                        "</span>",
                                    label: __("Create a new {0}", [label_name]),
