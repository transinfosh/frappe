...
frappe.ui.form.Form = class FrappeForm {
    ...
    watch_model_updates() {
        ...
        frappe.model.on(me.doctype, "*", function (fieldname, value, doc, skip_dirty_trigger = false) {
            ...
            if (doc.id == me.docid) {
                ...
                return me.script_manager.trigger(fieldname, doc.doctype, doc.id);
            }
        });
        ...
        $.each(table_fields, function (i, df) {
            frappe.model.on(df.options, "*", function (fieldname, value, doc, skip_dirty_trigger = false) {
                if (doc.parent == me.docid && doc.parentfield === df.fieldname) {
                    if (!skip_dirty_trigger) {
                        me.dirty();
                    }
    ...
    setup_notify_on_rename() {
        $(document).on("rename", (ev, dt, old_id, new_id) => {
            if (dt == this.doctype) this.rename_notify(dt, old_id, new_id);
        });

        frappe.realtime.on("doc_rename", (data) => {
            // the current form has been renamed by some backend process
            if (data.doctype == this.doctype && data.old == this.docid) {
    ...
    refresh(docid) {
        ...
        if (this.docid) {
            ...
            // set the doc
            this.doc = frappe.get_doc(this.doctype, this.docid);

            // check permissions
            this.fetch_permissions();
            if (!this.has_read_permission()) {
                frappe.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docid)));
                return;
            }
            ...
            if (!this.opendocs[this.docid]) {
                this.check_doctype_conflict(this.docid);
            } else {
    ...
    trigger_onload(switched) {
        if (!this.opendocs[this.docid]) {
            ...
            if (this.doc.localid) {
                // trigger form-rename and remove .localid
                delete this.doc.localid;
    ...
    initialize_new_doc() {
        ...
        // hide any open grid
        this.script_manager.trigger("before_load", this.doctype, this.docid).then(() => {
            me.script_manager.trigger("onload");
            me.opendocs[me.docid] = true;
            me.render_form();
        ...
        if (this.meta.track_seen) {
            $('.list-id[data-id="' + me.docid + '"]').addClass("seen");
        }
    }
    ...
    refresh_header(switched) {
        ...
        if (!this.meta.in_dialog || this.in_form) {
            frappe.utils.set_title(this.meta.issingle ? this.doctype : this.docid);
        }
    ...
    discard(btn, callback, on_error) {
        ...
        return new Promise((resolve) => {
            frappe.confirm(__("Discard {0}", [this.docid]), function () {
    ...
    savesubmit(btn, callback, on_error) {
        ...
        return new Promise((resolve) => {
            ...
            frappe.confirm(
                __("Permanently Submit {0}?", [this.docid]),
                function () {
                    frappe.validated = true;
        ...
        for (let doctype of doctypes) {
            if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
                let docids = links
                    ...
                    .map((link) => frappe.utils.get_form_link(link.doctype, link.id, true))
                    .join(", ");
                links_text += `<li><strong>${__(doctype)}</strong>: ${docids}</li>`;
            }
        }
    ...
    _cancel(btn, callback, on_error, skip_confirm) {
        ...
        if (skip_confirm) {
            ...
            frappe.confirm(__("Permanently Cancel {0}?", [this.docid]), cancel_doc, me.handle_save_fail(btn, on_error));
        }
    }
    ...
    _discard(btn, on_error, skip_confirm) {
        ...
        if (skip_confirm) {
            ...
            frappe.confirm(
                __("Permanently Discard {0}?", [this.docid]),
                discard_doc,
                me.handle_save_fail(btn, on_error)
            );
        }
    ...
    savetrash() {
        ...
        frappe.model.delete_doc(this.doctype, this.docid, function () {
            window.history.back();
        });
    }
    ...
    validate_form_action(action, resolve) {
        ...
        if (
            (frappe.workflow.is_read_only(this.doctype, this.docid) &&
    ...
    check_doctype_conflict(docid) {
        if (this.doctype == "DocType" && docid == "DocType") {
            frappe.msgprint(__("Allowing DocType, DocType. Be careful!"));
        } else if (this.doctype == "DocType") {
            if (frappe.views.formview[docid] || frappe.pages["List/" + docid]) {
    ...
    rename_notify(dt, old, id) {
        ...
        !frappe._from_link && frappe.set_route("Form", this.doctype, id);
    }
    ...
    print_doc() {
        ...
        frappe.set_route("print", this.doctype, this.doc.id);
    }
    ...
    rename_doc() {
        frappe.model.rename_doc(this.doctype, this.docid, () => this.refresh_header());
    }
    ...
    email_doc(message) {
        new frappe.views.CommunicationComposer({
            doc: this.doc,
            frm: this,
            subject: __(this.meta.id) + ": " + this.docid,
    ...
    reload_doc() {
        ...
        if (!this.doc.__islocal) {
            frappe.model.remove_from_locals(this.doctype, this.docid);
            return frappe.model.with_doc(this.doctype, this.docid, () => {
                this.refresh();
            });
        }
    }
    ...
    get_docinfo() {
        return frappe.model.docinfo[this.doctype][this.docid];
    }
    ...
    remove_custom_button(label, group) {
        ...
        let $btn = this.page.is_in_group_button_dropdown(this.page.menu, "li > a.grey-link > span", menu_item_label);
    ...
    get_doc() {
        return locals[this.doctype][this.docid];
    }
    ...
    set_currency_labels(fields_list, currency, parentfield) {
        ...
        $.each(fields_list, function (i, fname) {
            ...
            if (docfield) {
                var label = __(docfield.label || "", null, docfield.parent).replace(/\([^\)]*\)/g, ""); // eslint-disable-line
                if (parentfield) {
    ...
    field_map(fnames, fn) {
        ...
        for (var i = 0, l = fnames.length; i < l; i++) {
            ...
            var field = frappe.meta.get_docfield(this.doctype, fieldname, this.docid);
            if (field) {
                fn(field);
    ...
    get_docfield(fieldname1, fieldname2) {
        if (fieldname2) {
            ...
            return frappe.meta.get_docfield(doctype, fieldname2, this.docid);
        } else {
            // for parent
            return frappe.meta.get_docfield(this.doctype, fieldname1, this.docid);
        }
    }

    set_df_property(fieldname, property, value, docid, table_field, table_row_id = null) {
        let df;
        ...
        if (!docid || !table_field) {
            ...
            if (filtered_fields.length) {
                df = frappe.meta.get_docfield(filtered_fields[0].parent, table_field, table_row_id);
            }
        }
    ...
    set_value(field, value, if_missing, skip_dirty_trigger = false) {
        ...
        var _set = function (f, v) {
            ...
            if (fieldobj) {
                if (!if_missing || !frappe.model.has_value(me.doctype, me.doc.id, f)) {
                    if (frappe.model.table_fields.includes(fieldobj.df.fieldtype) && $.isArray(v)) {
                        ...
                        return frappe.model.set_value(me.doctype, me.doc.id, f, v, me.fieldtype, skip_dirty_trigger);
                    }
                }
            } else {
        ...
        if (!opts.doc) {
            ...
            opts.callback = function (r) {
                if ($.isPlainObject(r.message)) {
                    if (opts.child) {
                        // update child doc
                        opts.child = locals[opts.child.doctype][opts.child.id];
    ...
    trigger(event, doctype, docid) {
        return this.script_manager.trigger(event, doctype, docid);
    }

    get_formatted(fieldname) {
        return frappe.format(
            this.doc[fieldname],
            frappe.meta.get_docfield(this.doctype, fieldname, this.docid),
            { no_icon: true },
            this.doc
        );
    }
    ...
    get_selected() {
        // returns list of children that are selected. returns [parentfield, id] for each
        var selected = {},
            me = this;
    ...
    make_new(doctype) {
        ...
        if (this.make_methods && this.make_methods[doctype]) {
            ...
            frappe.model.with_doctype(doctype, function () {
                ...
                // frappe.set_route('Form', doctype, new_doc.id);
            });
        }
    }
    ...
    set_link_field(doctype, new_doc) {
        ...
        frappe.get_meta(doctype).fields.forEach(function (df) {
            if (df.fieldtype === "Link" && df.options === me.doctype) {
                new_doc[df.fieldname] = me.doc.id;
    ...
    update_in_all_rows(table_fieldname, fieldname, value) {
        ...
        frappe.model
            ...
            .filter((child) => !frappe.model.has_value(child.doctype, child.id, fieldname))
            .forEach((child) => frappe.model.set_value(child.doctype, child.id, fieldname, value));
    }
    ...
    setup_docinfo_change_listener() {
        ...
        if (this.doc && !this.is_new()) {
            frappe.realtime.doc_subscribe(doctype, docid);
        }
        ...
        frappe.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
            if (
                ...
                doc.reference_id !== docid
            ) {
                return;
            }
            let doc_list = frappe.model.docinfo[doctype][docid][key] || [];
            let docindex = doc_list.findIndex((old_doc) => {
                return old_doc.id === doc.id;
            });

            if (action === "add") {
                frappe.model.docinfo[doctype][docid][key].push(doc);
            }
            if (docindex > -1) {
                if (action === "update") {
                    frappe.model.docinfo[doctype][docid][key].splice(docindex, 1, doc);
                }
                if (action === "delete") {
                    frappe.model.docinfo[doctype][docid][key].splice(docindex, 1);
                }
            }
    ...
    set_fields_as_options(fieldname, reference_doctype, filter_function, default_options = [], table_fieldname) {
        ...
        return new Promise((resolve) => {
            frappe.model.with_doctype(reference_doctype, () => {
                ...
                options && this.set_df_property(fieldname, "options", options, this.doc.id, table_fieldname);
                resolve(options);
            });
        });
    }
    ...
    set_active_tab(tab) {
        const previous_tab_name = this.active_tab_map?.[this.docid]?.df?.fieldname || "";
        ...
        this.active_tab_map ??= {};
        this.active_tab_map[this.docid] = tab;
    ...
    get_active_tab() {
        return this.active_tab_map && this.active_tab_map[this.docid];
    }
    ...
    show_submission_queue_banner() {
        ...
        frappe
            .call({
                ...
                args: { doctype: this.doctype, docid: this.docid },
            })
            .then((r) => {
