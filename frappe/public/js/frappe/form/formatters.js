    ...
    Select: function (value, df) {
        ...
        for (var i = 0; i < options.length; i++) {
            ...
            if (df.options_has_label) {
                ...
                if (comma_index === 0) {
                    if (!value) {
                        return __(opt.substring(1));
                    }
                } else if (comma_index > 0) {
                    if (value === opt.substring(0, comma_index)) {
                        return __(opt.substring(comma_index + 1));
                    }
                }
            }
            else if (opt == value) {
    ...
    Rating: function (value, docfield) {
        ...
        Array.from({ length: cint(number_of_stars) }, (_, i) => i + 1).forEach((i) => {
            rating_html += `<svg class="icon icon-md" data-rating=${i} viewBox="0 0 24 24" fill="none">
                <path class="right-half ${i <= (value || 0) ? "star-click" : ""
                ...
                <path class="left-half ${i <= (value || 0) || i - 0.5 == value ? "star-click" : ""
