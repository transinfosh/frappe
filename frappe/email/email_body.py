...
class EMail:
    ...
    def add_pdf_attachment(self, id, html, options=None):
        self.add_attachment(id, get_pdf(html, options), "application/octet-stream")
    ...
    def replace_sender(self):
        if cint(self.email_account.always_use_account_email_id_as_sender):
            ...
            self.sender = email.utils.formataddr(
                (str(Header(sender_name or self.email_account.id, "utf-8")), self.email_account.email_id)
            )
    ...
    def replace_sender_name(self):
        if cint(self.email_account.always_use_account_name_as_sender_name):
            ...
            self.sender = email.utils.formataddr((str(Header(self.email_account.id, "utf-8")), sender_email))
...
def get_message_id():
    """Return Message ID created from doctype and id."""
