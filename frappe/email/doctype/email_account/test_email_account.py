...
class TestEmailAccount(IntegrationTestCase):
    ...
    def test_incoming(self):
        ...
        self.assertTrue(frappe.db.get_value(comm.reference_doctype, comm.reference_id, "id"))
        ...
        self.assertTrue(
            frappe.db.get_value(
                "Email Queue",
                {
                    ...
                    "reference_id": comm.reference_id,
                },
            )
        )
    ...
    def test_incoming_with_attach(self):
        ...
        frappe.delete_doc("File", existing_file.id)
        ...
        # check attachment
        attachments = get_attachments(comm.doctype, comm.id)
    ...
    def test_threading(self):
        ...
        sent_id = make(
            ...
            doctype="ToDo",
            id=frappe.get_last_doc("ToDo").id,
            send_email=True,
        )["id"]
        ...
        sent = frappe.get_doc("Communication", sent_id)

        comm = frappe.get_doc("Communication", {"sender": "test_sender@example.com"})
        self.assertEqual(comm.reference_doctype, sent.reference_doctype)
        self.assertEqual(comm.reference_id, sent.reference_id)
    ...
    def test_threading_by_subject(self):
        ...
        comm_list = frappe.get_all(
            ...
            fields=["id", "reference_doctype", "reference_id"],
        )
        ...
        self.assertEqual(comm_list[0].reference_id, comm_list[1].reference_id)
    ...
    def test_threading_by_message_id(self):
        ...
        last_mail = frappe.get_doc("Email Queue", dict(reference_id=event.id))
        ...
        self.assertEqual(comm_list[0].reference_id, event.id)
    ...
    def test_auto_reply(self):
        ...
        self.assertTrue(
            frappe.db.get_value(
                "Email Queue",
                {"reference_doctype": comm.reference_doctype, "reference_id": comm.reference_id},
            )
        )
    ...
    def test_append_to(self):
        ...
        self.assertTrue(communication.reference_id)
        self.assertTrue(frappe.db.exists(communication.reference_doctype, communication.reference_id))
...
class TestInboundMail(IntegrationTestCase):
    ...
    def test_mail_exist_validation(self):
        ...
        self.assertEqual(communication.id, new_communication.id)
    ...
    def test_find_parent_email_queue(self):
        ...
        self.assertEqual(queue_record.id, parent_queue.id)
    ...
    def test_find_parent_communication_through_queue(self):
        ...
        queue_record = self.new_email_queue(communication=communication.id)
        mail_content = self.get_test_mail(fname="reply-4.raw").replace(
            "{{ message_id }}", queue_record.message_id
        )
        ...
        self.assertEqual(parent_communication.id, communication.id)
    ...
    def test_find_parent_communication_from_header(self):
        """Incase of header contains parent communication id"""
        communication = self.new_communication()
        mail_content = self.get_test_mail(fname="reply-4.raw").replace(
            "{{ message_id }}", f"<{communication.id}@{frappe.local.site}>"
        )
    ...
    def test_reference_document(self):
        # Create email queue record
        todo = self.new_todo()
        # communication = self.new_communication(reference_doctype='ToDo', reference_id=todo.id)
        queue_record = self.new_email_queue(reference_doctype="ToDo", reference_id=todo.id)
        mail_content = self.get_test_mail(fname="reply-4.raw").replace(
            "{{ message_id }}", queue_record.message_id
        )
        ...
        self.assertEqual(todo.id, reference_doc.id)

    def test_reference_document_by_record_id_in_subject(self):
        # Create email queue record
        todo = self.new_todo()

        mail_content = self.get_test_mail(fname="incoming-subject-placeholder.raw").replace(
            "{{ subject }}", f"RE: (#{todo.id})"
        )
...
def cleanup(sender=None):
    ...
    ids = frappe.get_list("Communication", filters=filters, fields=["id"])
    for id in ids:
        frappe.delete_doc_if_exists("Communication", id.id)
        frappe.delete_doc_if_exists("Communication Link", {"parent": id.id})
