...
class EmailAccount(Document):
    ...
    def _disable_broken_incoming_account(self, description):
        ...
        users = get_system_managers(only_id=True)
        notification = {
            "document_type": self.doctype,
            "document_id": self.id,
            "subject": description,
            "from_user": "Administrator",
            "email_header": _("Email Account {0} Disabled").format(self.email_id or self.id),
        }
        enqueue_create_notification(users, notification)

    def set_failed_attempts_count(self, value):
        frappe.cache.set_value(f"{self.id}:email-account-failed-attempts", value, expires_in_sec=2 * 60 * 60)
