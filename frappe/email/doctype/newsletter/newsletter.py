...
class Newsletter(WebsiteGenerator):
    ...
    def validate(self):
        self.route = f"newsletters/{self.id}"
    ...
    def get_sending_status(self):
        ...
        emails_queued = is_job_queued(
            job_name=frappe.utils.get_job_name("send_bulk_emails_for", self.doctype, self.id),
            queue="long",
        )
    ...
    def get_email_groups(self) -> list[str]:
        ...
        return [x.email_group for x in self.email_group] or frappe.get_all(
            ...
            filters={"parent": self.id, "parenttype": "Newsletter"},
            pluck="email_group",
        )
...
def send_scheduled_email():
    ...
    for newsletter_id in scheduled_newsletter:
        try:
            newsletter = frappe.get_doc("Newsletter", newsletter_id)
            ...
            frappe.db.set_value("Newsletter", newsletter_id, "email_sent", 0)
...
@frappe.whitelist(allow_guest=True)
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_id=None):
    if not (recipient_email and reference_id):
        return
    verify_request()
    try:
        doc = frappe.get_cached_doc("Newsletter", reference_id)
