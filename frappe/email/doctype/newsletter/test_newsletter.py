...
class TestNewsletterMixin:
    ...
    def get_newsletter(**kwargs) -> "Newsletter":
        ...
        similar_newsletters = frappe.get_all(doctype, newsletter_content, pluck="id")
        ...
        newsletter.reload()
        newsletters.append(newsletter.id)

        attached_files = frappe.get_all(
            "File",
            {
                "attached_to_doctype": newsletter.doctype,
                "attached_to_id": newsletter.id,
            },
            pluck="id",
        )
...
class TestNewsletter(TestNewsletterMixin, IntegrationTestCase):
    def test_send(self):
        ...
        email_queue_list = [frappe.get_doc("Email Queue", e.id) for e in frappe.get_all("Email Queue")]
    ...
    def test_unsubscribe(self):
        id = self.send_newsletter()
        to_unsubscribe = choice(emails)
        group = frappe.get_all("Newsletter Email Group", filters={"parent": id}, fields=["email_group"])
    ...
    def test_retry_partially_sent_newsletter(self):
        ...
        queue_recipient_id = email_queue_list[0].recipients[0].recipient
        ...
        self.assertEqual(newsletter.get_pending_recipients(), [queue_recipient_id])
