...
def process_auto_email_report(report):
    ...
    try:
        ...
        auto_email_report.log_error(f"Failed to send {auto_email_report.id} Auto Email Report")
    ...
    for row in data:
        ...
        for col in columns:
            ...
            if col.fieldtype == "Link":
                ...
                doc = None
                if doc_id and col.get("parent") and not frappe.get_meta(col.parent).istable:
