...
class TestEmailQueue(IntegrationTestCase):
    def test_email_queue_deletion_based_on_modified_date(self):
        ...
        self.assertFalse(frappe.db.exists("Email Queue", old_record.id))
        self.assertFalse(frappe.db.exists("Email Queue Recipient", {"parent": old_record.id}))

        self.assertTrue(frappe.db.exists("Email Queue", new_record.id))
        self.assertTrue(frappe.db.exists("Email Queue Recipient", {"parent": new_record.id}))
    ...
    def test_failed_email_notification(self):
        ...
        email_record.reference_id = "Administrator"
        email_record.insert()
