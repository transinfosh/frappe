...
class SendMailContext:
    ...
    def notify_failed_email(self):
        ...
        notification.document_id = self.queue_doc.id
    ...
    def get_tracker_str(self, recipient_email) -> str:
        ...
        if self.queue_doc.get("email_read_tracker_url"):
            ...
            tracker_url = f"{get_url()}/api/method/frappe.core.doctype.communication.email.mark_email_as_seen?id={self.queue_doc.communication}"
    ...
    def include_attachments(self, message):
        ...
        for attachment in attachments:
            ...
            if attachment.get("fid"):
                file_filters["id"] = attachment.get("fid")
    ...
    def _store_file(self, file_name, content):
        ...
        if self.queue_doc.communication:
            ...
            file_data.attached_to_id = self.queue_doc.communication
        else:
            file_data.attached_to_doctype = self.queue_doc.doctype
            file_data.attached_to_id = self.queue_doc.id
...
def retry_sending(queues: str | list[str]):
    ...
    ).where(email_queue.id.isin(queues) & email_queue.status == "Error").run()


@frappe.whitelist()
def send_now(id, force_send: bool = False):
...
def on_doctype_update():
    """Add index in `tabCommunication` for `(reference_doctype, reference_id)`"""
...
class QueueBuilder:
    ...
    def __init__(
        ...
        :param reference_id: Reference id of caller document.
        :param send_priority: Priority for Email Queue, default 1.
        :param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/frappe.email.queue.unsubscribe`.
        :param unsubscribe_params: additional params for unsubscribed links. default are id, doctype, email
                ...
                .where(
                    ...
                    & (
                        (
                            ...
                            & (EmailUnsubscribe.reference_id == self.reference_id)
                        )
            ...
            for recipients in frappe.utils.create_batch(final_recipients, 1000):
                frappe.enqueue(
                    ...
                    job_name=frappe.utils.get_job_name(
                        "send_bulk_emails_for", self.reference_doctype, self.reference_id
                    ),
    ...
    def as_dict(self, include_recipients=True):
        email_account = self.get_outgoing_email_account()
        email_account_name = email_account and email_account.is_exists_in_db() and email_account.id
