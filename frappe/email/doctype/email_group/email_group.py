...
class EmailGroup(Document):
    ...
    def onload(self):
        singles = [d.id for d in frappe.get_all("DocType", "id", {"issingle": 1})]
    ...
    def import_from(self, doctype):
        ...
        for user in frappe.get_all(doctype, [email_field, unsubscribed_field or "id"]):
    ...
    def get_welcome_url(self, email: str | None = None) -> str | None:
        ...
        return (
            add_query_params(self.welcome_url, {"email": email, "email_group": self.id})
...
@frappe.whitelist()
def import_from(id, doctype):
    nlist = frappe.get_doc("Email Group", id)
    if nlist.has_permission("write"):
        return nlist.import_from(doctype)


@frappe.whitelist()
def add_subscribers(id, email_list):
    if not isinstance(email_list, list | tuple):
        email_list = email_list.replace(",", "\n").split("\n")

    template = frappe.db.get_value("Email Group", id, "welcome_email_template")
    ...
    for email in email_list:
        ...
        if parsed_email:
            if not frappe.db.get_value("Email Group Member", {"email_group": id, "email": parsed_email}):
                frappe.get_doc(
                    {"doctype": "Email Group Member", "email_group": id, "email": parsed_email}
                ).insert(ignore_permissions=frappe.flags.ignore_permissions)

                send_welcome_email(welcome_email, parsed_email, id)
    ...
    return frappe.get_doc("Email Group", id).update_total_subscribers()
