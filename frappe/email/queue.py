...
def unsubscribe(doctype, id, email):
    ...
    return_unsubscribed_page(email, doctype, id)


def return_unsubscribed_page(email, doctype, id):
    frappe.respond_as_web_page(
        _("Unsubscribed"),
        _("{0} has left the conversation in {1} {2}").format(email, _(doctype), id),
...
def flush():
    ...
    for row in email_queue_batch:
        try:
            email_queue: EmailQueue = frappe.get_doc("Email Queue", row.id)
            email_queue.send()
        except Exception:
            frappe.get_doc("Email Queue", row.id).log_error()
            failed_email_queues.append(row.id)
