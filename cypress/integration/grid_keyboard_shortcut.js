context("Grid Keyboard Shortcut", () => {
    ...
    beforeEach(() => {
        ...
        // as new ids uses hash instead of numbers get row's data-id dynamically.
        cy.get('.frappe-control[data-fieldname="email_ids"]')
            .find(".grid-body .grid-row")
            .should(($row) => {
                contact_email_name = $row.attr("data-id");
            });
    });
    it("Insert new row at the end", () => {
        cy.add_new_row_in_grid(
            ...
            (cy, total_count) => {
                cy.get(`[data-id="${contact_email_name}"]`).should(
                    "have.attr",
                    "data-idx",
                    `${total_count + 1}`
                );
    ...
    it("Insert new row at the top", () => {
        cy.add_new_row_in_grid("{ctrl}{shift}{uparrow}", (cy) => {
            cy.get(`[data-id="${contact_email_name}"]`).should("have.attr", "data-idx", "2");
        });
    });
    it("Insert new row below", () => {
        cy.add_new_row_in_grid("{ctrl}{downarrow}", (cy) => {
            cy.get(`[data-id^="${contact_email_name}"]`).should("have.attr", "data-idx", "1");
        });
    });
